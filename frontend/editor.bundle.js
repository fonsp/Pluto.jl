...
const initial_notebook = ()=>{
    ...
    ;
    const cells = [
        Cell(`html"<h1>Welcome to <em>WebAssembly</em>!</h1>\n\n<p>This is a first demo of (non-reactive) Pluto with a WASM backend! The Julia code in this notebook was executed <em>entirely</em> inside your web browser.</p>"`, true),
        Cell(`\n# hello from WASM!\n\nsqrt.(1:10)`, false),
        Cell(`\nextrema(1:100)`, false),
...
const applyPatches1 = vn;
const handle_message = async (on_unrequested_update, message_type, body = {
}, metadata = {
}, no_broadcast = true)=>{
    if (message_type === "update_notebook") {
        ...
        if (backend_notebook == null) {
            ...
            backend_notebook = applyPatches1(backend_notebook, body.updates);
            ...
            update_notebook1(on_unrequested_update, (notebook)=>{
                ...
                try {
                    ...
                    notebook.cell_results[cell_id] = {
                        queued: false,
                        running: false,
                        runtime: 1000000 * (Date.now() - start_time),
                        errored: false,
                        output: {
                            body: repred,
                            persist_js_state: false,
...
;
const sum = (values)=>values.reduce((a5, b2)=>a5 + b2
    , 0)
;
const finalize_statistics = async (state, client, counter_statistics)=>{
    ...
    const statistics = {
        ...
        numErrored: cell_results.filter((c5)=>c5.errored
        ).length,
        numFolded: cells.filter((c5)=>c5.code_folded
        ).length,
        ...
        numMarkdowns: cell_inputs_local.filter((c5)=>first_line(c5).startsWith('md"')
        ).length,
        numBinds: sum(cell_inputs_local.map((c5)=>count_matches(/\@bind/g, c5.code)
        )),
        numBegins: cell_inputs_local.filter((c5)=>first_line(c5).endsWith("begin")
        ).length,
        numLets: cell_inputs_local.filter((c5)=>first_line(c5).endsWith("let")
        ).length,
        cellSizes: value_counts(cell_inputs_local.map((c5)=>count_matches(/\n/g, c5.code) + 1
        )),
        runtimes: value_counts(cell_results.map((c5)=>Math.floor(Math.log10(c5.runtime + 1))
        )),
...
const get_unique_short_id1 = ()=>crypto.getRandomValues(new Uint32Array(1))[0].toString(36)
;
...
const create_pluto_connection1 = async ({ on_unrequested_update , on_reconnect , on_connection_status , connect_metadata ={
}  })=>{
    var ws_connection = null;
    const client = {
        send: null,
    ...
    const client_id = get_unique_short_id1();
    const sent_requests = {
    };
    const send = (message_type, body = {
    }, metadata = {
    }, no_broadcast = true)=>{
        const request_id = get_unique_short_id1();
        const message = {
            type: message_type,
            client_id: client_id,
...
class Editor extends d4 {
                ...
                switch(update.type){
                    case "notebook_diff":
                        if (message?.response?.from_reset) {
                            ...
                            try {
                                apply_notebook_patches(message.patches, initial_notebook1());
                            } catch (exception) {
    ...
    render() {
        ...
        }}\n                                placeholder="Save notebook..."\n                                button_label=${notebook.in_temp_dir ? "Choose" : "Move"}\n                            />\n                            <button class="toggle_export" title="Export..." onClick=${()=>this.setState({
                export_menu_open: !export_menu_open
            })
        }>\n                                <span></span>\n                            </button>\n                            <div id="process_status">${statusval === "disconnected" ? "Reconnecting..." : status.loading && status.wasm ? "Loading WASM..." : statusval === "loading" ? "Loading..." : null}</div>\n                        </nav>\n                    </header>\n                    <${Main}>\n                        <preamble>\n                            <button\n                                onClick=${()=>{
