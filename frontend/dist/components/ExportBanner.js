...
exports.ExportBanner = exports.WarnForVisisblePasswords = void 0;
//@ts-ignore
...
const Circle = ({ fill }) => (0, Preact_js_1.html) `
    <svg
        width="48"
        height="48"
        viewBox="0 0 48 48"
...
`;
const Triangle = ({ fill }) => (0, Preact_js_1.html) `
...
`;
const Square = ({ fill }) => (0, Preact_js_1.html) `
    <svg width="48" height="48" viewBox="0 0 48 48" style="height: .7em; width: .7em; margin-left: .3em; margin-right: .2em; margin-bottom: -.1em;">
        <polygon points="0,0 0,40 40,40 40,0" fill=${fill} stroke="none" />
    </svg>
`;
const WarnForVisisblePasswords = () => {
    if (Array.from(document.querySelectorAll("bond")).some((bond_el) => Array.from(bond_el.querySelectorAll(`input[type="password"]`)).some((input) => {
        // @ts-ignore
        if (input?.value !== "") {
        ...
        alert("Warning: this notebook includes a password input with something typed in it. The contents of this password field will be included in the exported file in an unsafe way. \n\nClear the password field and export again to avoid this problem.");
    }
};
exports.WarnForVisisblePasswords = WarnForVisisblePasswords;
const ExportBanner = ({ notebook_id, print_title, open, onClose, notebookfile_url, notebookexport_url, start_recording }) => {
    ...
    //
    let print_old_title_ref = (0, Preact_js_1.useRef)("");
    (0, useEventListener_js_1.useEventListener)(window, "beforeprint", (e) => {
        if (!e.detail?.fake) {
            print_old_title_ref.current = document.title;
            document.title = print_title.replace(/\.jl$/, "").replace(/\.plutojl$/, "");
        }
    }, [print_title]);
    (0, useEventListener_js_1.useEventListener)(window, "afterprint", () => {
        document.title = print_old_title_ref.current;
    }, [print_title]);
    const element_ref = (0, Preact_js_1.useRef)(/** @type {HTMLDialogElement?} */ (null));
    (0, Preact_js_1.useLayoutEffect)(() => {
        if (element_ref.current != null && typeof HTMLDialogElement !== "function")
            dialog_polyfill_esm_min_js_1.default.registerDialog(element_ref.current);
    }, [element_ref.current]);
    (0, Preact_js_1.useLayoutEffect)(() => {
    ...
    const onCloseRef = (0, Preact_js_1.useRef)(onClose);
    onCloseRef.current = onClose;
    (0, useEventListener_js_1.useEventListener)(element_ref.current, "focusout", () => {
    ...
    return (0, Preact_js_1.html) `
        <dialog id="export" inert=${!open} open=${open} ref=${element_ref} class=${prideMonth ? "pride" : ""}>
            <div id="container">
                ...
                <a
                    ...
                    download=""
                    onClick=${(e) => {
        (0, exports.WarnForVisisblePasswords)();
        exportNotebook(e, 1);
    }}
                >
        ...
        e.preventDefault();
                ...
                `}
                ${prideMonth
        ? (0, Preact_js_1.html) `<div class="pride_message">
...
exports.ExportBanner = ExportBanner;
