...
exports.pluto_autocomplete = void 0;
...
const LiveDocsFromCursor_js_1 = require("./LiveDocsFromCursor.js");
const ClassTable_js_1 = require("../../common/ClassTable.js");
const scopestate_statefield_js_1 = require("./scopestate_statefield.js");
const BottomRightPanel_js_1 = require("../BottomRightPanel.js");
const CellInput_js_1 = require("../CellInput.js");
const go_to_definition_plugin_js_1 = require("./go_to_definition_plugin.js");
const mixedParsers_js_1 = require("./mixedParsers.js");
const UnicodeTools_js_1 = require("../../common/UnicodeTools.js");
let { autocompletion, completionKeymap, completionStatus, acceptCompletion, selectedCompletion } = CodemirrorPlutoSetup_js_1.autocomplete;
...
const tab_completion_command = (cm) => {
    ...
    if (last_char === ")" && !last_line.includes("?"))
        return false;
    return CodemirrorPlutoSetup_js_1.autocomplete.startCompletion(cm);
};
...
let open_docs_if_autocomplete_is_open_command = (cm) => {
    if (CodemirrorPlutoSetup_js_1.autocomplete.completionStatus(cm.state) != null) {
...
let update_docs_from_autocomplete_selection = (on_update_doc_query) => {
    ...
    return CodemirrorPlutoSetup_js_1.EditorView.updateListener.of((update) => {
        ...
        if (typeof text_to_apply !== "string")
            return;
        ...
        if (active_result?.hasResult?.() !== true)
            return; // not an ActiveResult instance
        const from = active_result.from, to = Math.min(active_result.to, update.state.doc.length);
        ...
        let docs_string = (0, LiveDocsFromCursor_js_1.get_selected_doc_from_state)(result_transaction.state);
        if (docs_string != null) {
...
function match_string_complete(state, pos) {
    ...
    if (node == null || !mixedParsers_js_1.STRING_NODE_NAMES.has(node.name)) {
        return false;
    }
    return true;
}
...
const not_explicit_and_too_boring = (/** @type {autocomplete.CompletionContext} */ ctx, allow_strings = false) => {
    ...
    if (ctx.matchBefore(/[ =)+-/,*:'"]$/))
        return true;
    if (ctx.tokenBefore(["IntegerLiteral", "FloatLiteral", "LineComment", "BlockComment", "Symbol"]) != null)
        return true;
    if (!allow_strings) {
        if (ctx.tokenBefore([...mixedParsers_js_1.STRING_NODE_NAMES]) != null) {
...
const julia_code_completions_to_cm = (/** @type {PlutoRequestAutocomplete} */ request_autocomplete) => 
/** @returns {Promise<autocomplete.CompletionResult?>} */
async (/** @type {autocomplete.CompletionContext} */ ctx) => {
    if (match_latex_symbol_complete(ctx))
        return null;
    if (!ctx.explicit && writing_variable_name_or_keyword(ctx))
        return null;
    if (not_explicit_and_too_boring(ctx))
        return null;
    ...
    const globals = ctx.state.facet(go_to_definition_plugin_js_1.GlobalDefinitionsFacet);
    ...
    const result = {
        ...
        options: [
            ...results
                .filter(([text, _1, _2, is_from_notebook, completion_type]) => (ctx.explicit || completion_type != "path") && (ctx.explicit || completion_type != "method") && !is_already_a_global(text))
                ...
                let text_to_apply = completion_type === "method" ? to_complete : is_field_expression ? override_text_to_apply_in_field_expression(text) ?? text : text;
                value_type = value_type === "Function" && text.startsWith("@") ? "Macro" : value_type;
                return {
                    label: text,
                    apply: text_to_apply,
                    type: (0, ClassTable_js_1.cl)({
                    ...
                    boost: completion_type === "keyword_argument" ? 7 : is_field_expression ? field_rank_heuristic(text_to_apply, is_exported) : undefined,
...
const complete_anyword = async (/** @type {autocomplete.CompletionContext} */ ctx) => {
    ...
    if (not_explicit_and_too_boring(ctx))
        return null;
    const results_from_cm = await CodemirrorPlutoSetup_js_1.autocomplete.completeAnyWord(ctx);
    if (results_from_cm === null)
        return null;
    return {
...
const writing_variable_name_or_keyword = (/** @type {autocomplete.CompletionContext} */ ctx) => {
    ...
    if (inside_do_argument_expression)
        return true;
    let node = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(ctx.state).resolve(ctx.pos, -1);
    let npn = node?.parent?.name;
    if (node?.name === "Identifier" && npn === "StructDefinition")
        return true;
    if (node?.name === "Identifier" && npn === "KeywordArguments")
        return true;
    ...
    if (inside_assigment_lhs)
        return true;
    return false;
};
const global_variables_completion = (/** @type {() => { [uuid: String]: String[]}} */ request_unsubmitted_global_definitions, cell_id) => 
/** @returns {Promise<autocomplete.CompletionResult?>} */
async (/** @type {autocomplete.CompletionContext} */ ctx) => {
    if (ctx.matchBefore(/[(\p{L}\p{Nl}\p{Sc}\d_!]$/u) == null)
        return null;
    ...
    if (there_is_a_dot_before)
        return null;
    const globals = ctx.state.facet(go_to_definition_plugin_js_1.GlobalDefinitionsFacet);
    const local_globals = request_unsubmitted_global_definitions();
    const possibles = lodash_js_1.default.union(
    ...
    ...Object.values(lodash_js_1.default.omit(local_globals, cell_id)));
    return await make_it_julian(CodemirrorPlutoSetup_js_1.autocomplete.completeFromList(possibles.map((label) => {
        return {
            label,
            apply: label,
...
const endswith_keyword_regex = /^(.*\s)?(baremodule|begin|break|catch|ccall|const|continue|do|else|elseif|end|export|false|finally|for|function|global|if|import|let|local|macro|module|quote|return|struct|true|try|type|using|while)$/;
...
const keyword_completions_generator = make_it_julian(CodemirrorPlutoSetup_js_1.autocomplete.completeFromList(keyword_completions));
...
const complete_package_name = (/** @type {() => Promise<string[]>} */ request_packages) => {
    ...
    return async (/** @type {autocomplete.CompletionContext} */ ctx) => {
        ...
        if (ctx.matchBefore(/[ ,][a-zA-Z0-9]+$/) == null)
            return null;
        if (ctx.tokenBefore(["Identifier"]) == null)
            return null;
        const tree = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(ctx.state);
        const node = tree.resolve(ctx.pos, -1);
        if (!(node.matchContext(["UsingStatement", "ImportPath"]) || node.matchContext(["ImportStatement", "ImportPath"])))
            return null;
        const packages = await get_packages();
        return await make_it_julian(CodemirrorPlutoSetup_js_1.autocomplete.completeFromList(packages))(ctx);
    };
};
const local_variables_completion = async (/** @type {autocomplete.CompletionContext} */ ctx) => {
    let scopestate = ctx.state.field(scopestate_statefield_js_1.ScopeStateField);
    ...
    return await make_it_julian(CodemirrorPlutoSetup_js_1.autocomplete.completeFromList(possibles))(ctx);
};
...
const apply_completion = (view, completion, from, to) => {
    ...
    view.dispatch({
        ...CodemirrorPlutoSetup_js_1.autocomplete.insertCompletionText(view.state, insert, from, to),
        annotations: CodemirrorPlutoSetup_js_1.autocomplete.pickedCompletion.of(completion),
    });
};
const special_symbols_completion = (/** @type {() => Promise<SpecialSymbols?>} */ request_special_symbols) => {
        ...
        if (list == null) {
            list = request_special_symbols().then((data) => {
                if (data != null) {
                    ...
                    return [emoji, latex].flatMap((map) => Object.entries(map).map(([label, value]) => {
                        return {
                            label,
    ...
    return async (/** @type {autocomplete.CompletionContext} */ ctx) => {
        ...
        if (!ctx.explicit && writing_variable_name_or_keyword(ctx))
            return null;
        if (not_explicit_and_too_boring(ctx, true))
            return null;
        return await CodemirrorPlutoSetup_js_1.autocomplete.completeFromList((await get_list()) ?? [])(ctx);
    };
};
const superscript_subscript_completion = () => {
    const match_sup = new RegExp(`\\\\\\\^([${Object.keys(UnicodeTools_js_1.sup_charmap)
        .map((x) => (x.match(/[\w\d]/) ? x : `\\${x}`))
        .join("")}]{2,})$`);
    const match_sub = new RegExp(`\\\\\\\_([${Object.keys(UnicodeTools_js_1.sub_charmap)
        .map((x) => (x.match(/[\w\d]/) ? x : `\\${x}`))
        .join("")}]{2,})$`);
    return (/** @type {autocomplete.CompletionContext} */ ctx) => {
        ...
        let dict = sup_match_result != null ? UnicodeTools_js_1.sup_charmap : UnicodeTools_js_1.sub_charmap;
        if (match_result) {
 ...
 */
let pluto_autocomplete = ({ request_autocomplete, request_special_symbols, request_packages, on_update_doc_query, request_unsubmitted_global_definitions, cell_id, }) => {
    let last_query = null;
    ...
    let memoize_last_request_autocomplete = async (options) => {
        if (lodash_js_1.default.isEqual(options, last_query)) {
    ...
    return [
        autocompletion({
            activateOnTyping: CellInput_js_1.ENABLE_CM_AUTOCOMPLETE_ON_TYPE,
            override: [
        ...
        CodemirrorPlutoSetup_js_1.keymap.of(pluto_autocomplete_keymap),
        CodemirrorPlutoSetup_js_1.keymap.of(completionKeymap),
    ];
};
exports.pluto_autocomplete = pluto_autocomplete;
