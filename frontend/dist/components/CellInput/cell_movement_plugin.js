...
exports.cell_movement_plugin = exports.prevent_holding_a_key_from_doing_things_across_cells = void 0;
...
let cell_movement_keys = ({ focus_on_neighbor }) => {
    ...
    const CellArrowLeft = (/** @type {EditorView} */ view) => {
        ...
        if (!selection.empty)
            return false;
        if (!view.moveByChar(selection, false).eq(selection))
            return false;
        focus_on_neighbor({
            cell_delta: -1,
            line: Infinity,
    ...
    const CellArrowRight = (/** @type {EditorView} */ view) => {
        ...
        if (!selection.empty)
            return false;
        if (!view.moveByChar(selection, true).eq(selection))
            return false;
        focus_on_neighbor({
            cell_delta: 1,
            line: 0,
    ...
    const CellArrowDown = (/** @type {EditorView} */ view) => {
        ...
        if (!selection.empty)
            return false;
        if (!view.moveVertically(selection, true).eq(selection))
            return false;
        focus_on_neighbor({
            cell_delta: 1,
            line: 0,
    ...
    return CodemirrorPlutoSetup_js_1.keymap.of([
...
exports.prevent_holding_a_key_from_doing_things_across_cells = CodemirrorPlutoSetup_js_1.EditorView.domEventHandlers({
    keydown: (event, view) => {
        ...
        let autocompletion_open = CodemirrorPlutoSetup_js_1.autocomplete.completionStatus(view.state) === "active";
        // If we have a cursor instead of a multicharacter selection:
        if (event.key === "ArrowUp" && !autocompletion_open) {
            if (!view.moveVertically(view.state.selection.main, false).eq(selection))
                return false;
            return true;
        }
...
exports.cell_movement_plugin = cell_movement_plugin;
