...
exports.go_to_definition_plugin = exports.GlobalDefinitionsFacet = void 0;
...
let get_variable_marks = (state, { scopestate, global_definitions }) => {
    return CodemirrorPlutoSetup_js_1.Decoration.set(filter_non_null(scopestate.usages.map(({ definition, usage, name }) => {
        if (definition == null) {
            ...
            if (global_definitions[name]) {
                return CodemirrorPlutoSetup_js_1.Decoration.mark({
                    tagName: "a",
                    attributes: {
                        "title": `${KeyboardShortcuts_js_1.ctrl_or_cmd_name}-Click to jump to the definition of ${name}.`,
 ...
 */
exports.GlobalDefinitionsFacet = CodemirrorPlutoSetup_js_1.Facet.define({
    combine: (values) => values[0],
    compare: lodash_js_1.default.isEqual,
});
exports.go_to_definition_plugin = CodemirrorPlutoSetup_js_1.ViewPlugin.fromClass(class {
    /**
     * @param {EditorView} view
     */
    constructor(view) {
        let global_definitions = view.state.facet(exports.GlobalDefinitionsFacet);
        this.decorations = get_variable_marks(view.state, {
            scopestate: view.state.field(scopestate_statefield_js_1.ScopeStateField),
            global_definitions,
        });
    }
    update(update) {
        // My best take on getting this to update when GlobalDefinitionsFacet does 🤷‍♀️
        let global_definitions = update.state.facet(exports.GlobalDefinitionsFacet);
        if (update.docChanged || update.viewportChanged || global_definitions !== update.startState.facet(exports.GlobalDefinitionsFacet)) {
            this.decorations = get_variable_marks(update.state, {
                scopestate: update.state.field(scopestate_statefield_js_1.ScopeStateField),
                global_definitions,
            });
        }
    }
        ...
        click: (event, view) => {
            if (event.target instanceof Element) {
                ...
                if (pluto_variable) {
                    ...
                    if (!((0, KeyboardShortcuts_js_1.has_ctrl_or_cmd_pressed)(event) || view.state.readOnly)) {
                        return false;
                    }
