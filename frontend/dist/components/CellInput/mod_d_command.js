...
exports.mod_d_command = {
    ...
    run: ({ state, dispatch }) => {
        if (state.selection.main.empty) {
            ...
            let cursor_left = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(state).cursorAt(state.selection.main.from, -1);
            let cursor_right = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(state).cursorAt(state.selection.main.from, 1);
            ...
            (0, CodemirrorPlutoSetup_js_1.selectNextOccurrence)({ state, dispatch });
        }
        return false;
    },
    /** @param {EditorView} view */
    shift: ({ state, dispatch }) => {
        ...
        dispatch({
            selection: CodemirrorPlutoSetup_js_1.EditorSelection.create(new_ranges, new_main_index),
            effects: previous_selected == null ? [] : CodemirrorPlutoSetup_js_1.EditorView.scrollIntoView(previous_selected.from),
        });
        return true;
    },
    preventDefault: true,
