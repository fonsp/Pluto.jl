...
exports.ScopeStateField = exports.explore_variable_usage = void 0;
...
const does_this_create_scope = (/** @type {TreeCursor} */ cursor) => {
    if (HardScopeNames.has(cursor.name))
        return true;
...
const back_to_parent_resetter = (/** @type {TreeCursor} */ cursor) => {
    const map = new CodemirrorPlutoSetup_js_1.NodeWeakMap();
    map.cursorSet(cursor, "here");
    return () => {
        while (map.cursorGet(cursor) !== "here") {
            if (!cursor.parent())
                throw new Error("Could not find my back to the original parent!");
        }
    };
};
...
const i_am_nth_child = (cursor) => {
    ...
    map.cursorSet(cursor, "here");
    if (!cursor.parent())
        throw new Error("Cannot be toplevel");
    cursor.firstChild();
    let i = 0;
    while (map.cursorGet(cursor) !== "here") {
        i++;
...
const explore_funcdef_arguments = (cursor, { enter, leave }) => {
    ...
    if (!cursor.firstChild())
        throw new Error(`Expected to go into function definition argument expression, stuck at ${cursor.name}`);
...
let explore_variable_usage = (tree, doc, _scopestate, verbose = VERBOSE) => {
    ...
    const return_false_immediately = new CodemirrorPlutoSetup_js_1.NodeWeakMap();
    let enter, leave;
    enter = (/** @type {TreeCursor} */ cursor) => {
        ...
        const register_variable = (range) => {
            ...
            if (local_scope_stack.length === 0)
                ...
                locals.push({ name, validity: lodash_js_1.default.last(local_scope_stack), definition: range });
        };
        ...
        if (cursor.name === "Identifier" || cursor.name === "MacroIdentifier" || cursor.name === "Operator") {
            ...
            if (cursor.matchContext(["FunctionDefinition", "Signature"]) || (cursor.matchContext(["Assignment"]) && i_am_nth_child(cursor) === 0)) {
                ...
                if (cursor.name === "Identifier" || cursor.name === "Operator" || cursor.name === "FieldExpression") {
                    if (verbose)
                        console.log("found function name", doc.sliceString(cursor.from, cursor.to), cursor.name);
                    const last_scoper = local_scope_stack.pop();
                    register_variable(r(cursor));
                    if (last_scoper)
                        local_scope_stack.push(last_scoper);
                    cursor.nextSibling();
                }
                if (verbose)
                    console.log("expl funcdef ", doc.sliceString(cursor.from, cursor.to));
                explore_funcdef_arguments(cursor, { enter, leave }).forEach(register_variable);
                if (verbose)
                    console.log("expl funcdef ", doc.sliceString(cursor.from, cursor.to));
                pos_resetter();
                if (verbose)
                    console.log("end of FunctionDefinition, currently at ", cursor.node);
                if (verbose)
                    console.groupEnd();
                return false;
            }
    ...
    if (local_scope_stack.length > 0)
        throw new Error(`Some scopes were not leaved... ${JSON.stringify(local_scope_stack)}`);
    const output = { usages, definitions, locals };
    if (verbose)
        console.log(output);
    return output;
};
exports.explore_variable_usage = explore_variable_usage;
/**
 * @type {StateField<ScopeState>}
 */
exports.ScopeStateField = CodemirrorPlutoSetup_js_1.StateField.define({
    create(state) {
        try {
            let cursor = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(state).cursor();
            let scopestate = (0, exports.explore_variable_usage)(cursor, state.doc, undefined);
            return scopestate;
        }
        catch (error) {
    ...
    update(value, tr) {
        try {
            if ((0, CodemirrorPlutoSetup_js_1.syntaxTree)(tr.state) != (0, CodemirrorPlutoSetup_js_1.syntaxTree)(tr.startState)) {
                let cursor = (0, CodemirrorPlutoSetup_js_1.syntaxTree)(tr.state).cursor();
                let scopestate = (0, exports.explore_variable_usage)(cursor, tr.state.doc, null);
                return scopestate;
            }
            else {
                return value;
