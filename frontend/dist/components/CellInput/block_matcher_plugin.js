...
const CodemirrorPlutoSetup_js_4 = require("../../imports/CodemirrorPlutoSetup.js");
/**
...
const baseTheme = CodemirrorPlutoSetup_js_3.EditorView.baseTheme({
    ".cm-matchingBracket": { backgroundColor: "#328c8252" },
    ".cm-nonmatchingBracket": { backgroundColor: "#bb555544" },
});
const DefaultScanDist = 10000, DefaultBrackets = "()[]{}";
const bracketMatchingConfig = CodemirrorPlutoSetup_js_1.Facet.define({
    combine(configs) {
        return (0, CodemirrorPlutoSetup_js_1.combineConfig)(configs, {
            afterCursor: true,
...
const matchingMark = CodemirrorPlutoSetup_js_4.Decoration.mark({ class: "cm-matchingBracket" }), nonmatchingMark = CodemirrorPlutoSetup_js_4.Decoration.mark({ class: "cm-nonmatchingBracket" });
const bracketMatchingState = CodemirrorPlutoSetup_js_1.StateField.define({
    create() {
        return CodemirrorPlutoSetup_js_4.Decoration.none;
    },
    update(deco, tr) {
        ...
        return CodemirrorPlutoSetup_js_4.Decoration.set(decorations, true);
    },
    provide: (f) => CodemirrorPlutoSetup_js_3.EditorView.decorations.from(f),
});
...
function matchBrackets(state, pos, dir, config = {}) {
    ...
    let tree = (0, CodemirrorPlutoSetup_js_2.syntaxTree)(state), node = tree.resolveInner(pos, dir);
...
function matchPlainBrackets(state, pos, dir, tree, tokenType, maxScanDistance, brackets) {
    ...
    for (let distance = 0; !iter.next().done && distance <= maxScanDistance;) {
        ...
        for (let pos = dir > 0 ? 0 : text.length - 1, end = dir > 0 ? text.length : -1; pos != end; pos += dir) {
            ...
            if (found < 0 || bracket_node_name_normalizer(tree.resolve(basePos + pos, 1).name) != tokenType)
                continue;
            if ((found % 2 == 0) == dir > 0) {
                depth++;
            }
