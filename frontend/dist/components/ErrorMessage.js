...
exports.ErrorMessage = exports.ParseError = void 0;
...
const FixWithAIButton_js_1 = require("./FixWithAIButton.js");
const nbsp = "\u00A0";
...
const focus_line = (cell_id, line) => window.dispatchEvent(new CustomEvent("cell_focus", {
    detail: {
        cell_id: cell_id,
        line: line,
    },
...
const DocLink = ({ frame }) => {
    ...
    if (extract_cell_id(frame.file))
        return null;
    if (frame.parent_module == null)
        return null;
    ...
    if (!builtin && nb?.nbpkg != null && !installed)
        return null;
    return (0, Preact_js_1.html) ` ${nbsp}<span
            ><a
                href="#"
                class="doclink"
                onClick=${(e) => {
...
const StackFrameFilename = ({ frame, cell_id }) => {
    ...
    if (frame_cell_id != null) {
        ...
        >
            ${frame_cell_id == cell_id ? "This\xa0cell" : "Other\xa0cell"}${noline(line) ? null : (0, Preact_js_1.html) `:${nbsp}<em>line${nbsp}${line}</em>`}
        </a>`;
    }
    else {
        const sp = frame.source_package;
        const origin = ["Main", "Core", "Base"].includes(sp) ? "julia" : sp;
        const file_line = (0, Preact_js_1.html) `<em>${frame.file.replace(/#@#==#.*/, "")}${noline(frame.line) ? null : `:${frame.line}`}</em>`;
        const text = sp != null ? (0, Preact_js_1.html) `<strong>${origin}</strong>${nbsp}→${nbsp}${file_line}` : file_line;
        const href = frame?.url?.startsWith?.("https") ? frame.url : null;
        return (0, Preact_js_1.html) `<a title=${frame.path} class="remote-url" href=${href}>${text}</a>`;
    }
};
const at = (0, Preact_js_1.html) `<span> from${nbsp}</span>`;
...
const Funccall = ({ frame }) => {
    let [expanded_state, set_expanded] = (0, Preact_js_1.useState)(false);
    ...
    const funcname_display = funcname.match(/^#\d+(#\d+)?$/)
        ? (0, Preact_js_1.html) `<abbr title="A (mini-)function that is defined without the 'function' keyword, but using -> or 'do'.">anonymous function</abbr>`
        : funcname;
    let inner = (0, Preact_js_1.html) `<strong>${funcname_display}</strong><${HighlightCallArgumentNames} code=${call_funcname_args[1]} />`;
    const id = (0, Preact_js_1.useMemo)(() => Math.random().toString(36).substring(7), [frame]);
    return (0, Preact_js_1.html) `<mark id=${id}>${inner}</mark> ${!expanded
    ...
    if (cell_id) {
        ...
        if (code) {
            ...
            return (0, Preact_js_1.html) `<a
                ...
                ><div>
                    <pre>
${lines.map((line, i) => frame.line - 1 - num_context_lines <= i && i <= frame.line - 1 + num_context_lines
                ? (0, Preact_js_1.html) `<${JuliaHighlightedLine} code=${line} i=${i} frameLine=${i === frame.line - 1} />`
                : null)}</pre
                    >
                </div></a
            >`;
...
const JuliaHighlightedLine = ({ code, frameLine, i }) => {
    const code_ref = (0, Preact_js_1.useRef)(/** @type {HTMLPreElement?} */ (null));
    (0, Preact_js_1.useLayoutEffect)(() => {
        if (code_ref.current) {
            code_ref.current.innerText = code;
            delete code_ref.current.dataset.highlighted;
            (0, CellOutput_js_1.highlight)(code_ref.current, "julia");
        }
    }, [code_ref.current, code]);
    return (0, Preact_js_1.html) `<code
        ref=${code_ref}
...
const HighlightCallArgumentNames = ({ code }) => {
    ...
    return (0, Preact_js_1.html) `<s-span ref=${code_ref} class="language-julia"></s-span>`;
};
...
const ParseError = ({ cell_id, diagnostics, last_run_timestamp }) => {
    (0, Preact_js_1.useEffect)(() => {
        window.dispatchEvent(new CustomEvent("cell_diagnostics", {
            detail: {
                cell_id,
                diagnostics,
            },
    ...
    return (0, Preact_js_1.html) `
        <jlerror class="syntax-error">
            <header>
                <p>Syntax error</p>
                <${FixWithAIButton_js_1.FixWithAIButton} cell_id=${cell_id} diagnostics=${diagnostics} last_run_timestamp=${last_run_timestamp} />
            </header>
            <section>
                ...
                <ol>
                    ${diagnostics.map(({ message, from, to, line }) => (0, Preact_js_1.html) `<li
                                class="from_this_notebook from_this_cell important"
                                onmouseenter=${() => cell_is_unedited(cell_id)
        ? window.dispatchEvent(new CustomEvent("cell_highlight_range", { detail: { cell_id, from, to } }))
        : null}
                                onmouseleave=${() => window.dispatchEvent(new CustomEvent("cell_highlight_range", { detail: { cell_id, from: null, to: null } }))}
                            >
...
const AnsiUpLine = (/** @type {{value: string}} */ { value }) => {
    ...
    const did_ansi_up = (0, Preact_js_1.useRef)(false);
    (0, Preact_js_1.useLayoutEffect)(() => {
        if (!node_ref.current)
            return;
    ...
    return value === "" ? (0, Preact_js_1.html) `<p><br /></p>` : (0, Preact_js_1.html) `<p ref=${node_ref}>${did_ansi_up.current ? null : without_ansi_chars}</p>`;
};
const ErrorMessage = ({ msg, stacktrace, plain_error, cell_id }) => {
    let pluto_actions = (0, Preact_js_1.useContext)(PlutoContext_js_1.PlutoActionsContext);
    const default_rewriter = {
        pattern: /.?/,
        display: (/** @type{string} */ x) => lodash_js_1.default.dropRightWhile(x.split("\n"), (s) => s === "").map((line) => (0, Preact_js_1.html) `<${AnsiUpLine} value=${line} />`),
    };
    const rewriters = [
        {
            pattern: /syntax: extra token after end of expression/,
            display: (/** @type{string} */ x) => {
                const begin_hint = (0, Preact_js_1.html) `<a
                    href="#"
                    onClick=${(e) => {
                    e.preventDefault();
                ...
                if (x.includes("\n\nBoundaries: ")) {
                    ...
                    const split_hint = (0, Preact_js_1.html) `<p>
                        <a
                            href="#"
                            onClick=${(e) => {
                        e.preventDefault();
                    ...
                    return (0, Preact_js_1.html) `<p>Multiple expressions in one cell.</p>
        ...
        },
        {
            ...
            display: () => (0, Preact_js_1.html) `<p>Tried to reevaluate an <code>include</code> call, this is not supported. You might need to restart this notebook from the main menu.</p>
                    <p>
        ...
        },
        {
            ...
            display: (/** @type{string} */ x) => x.split("\n").map((line) => (0, Preact_js_1.html) `<p style="white-space: nowrap;">${line}</p>`),
        },
        {
            ...
            display: (/** @type{string} */ x) => x.split("\n").map((line) => {
                ...
                if (match) {
                    ...
                    let symbol_links = syms.map((what) => (0, Preact_js_1.html) `<a href="#${encodeURI(what)}">${what}</a>`);
                    return (0, Preact_js_1.html) `<p>Cyclic references among${" "}${insert_commas_and_and(symbol_links)}.</p>`;
                }
                else {
                    return (0, Preact_js_1.html) `<p>${line}</p>`;
                }
            }),
        },
        {
            ...
            display: (/** @type{string} */ x) => x.split("\n").map((line) => {
                ...
                if (match) {
                    ...
                    let symbol_links = syms.map((what) => {
                        ...
                        return (0, Preact_js_1.html) `<a href="#" onclick=${onclick}>${what}</a>`;
                    });
                    return (0, Preact_js_1.html) `<p>Multiple definitions for${" "}${insert_commas_and_and(symbol_links)}.</p>`;
                }
                else {
                    return (0, Preact_js_1.html) `<p>${line}</p>`;
                }
        ...
        },
        {
            ...
            display: (/** @type{string} */ x) => {
                ...
                if (Object.keys(erred_upstreams).length === 0 || !undefvar_is_from_upstream) {
                    return (0, Preact_js_1.html) `<p>${x}</p>`;
                }
                const symbol_links = Object.keys(erred_upstreams).map((key) => {
                    ...
                    return (0, Preact_js_1.html) `<a href="#" onclick=${onclick}>${key}</a>`;
                });
                // const plural = symbol_links.length > 1
                return (0, Preact_js_1.html) `<p><em>Another cell defining ${insert_commas_and_and(symbol_links)} contains errors.</em></p>`;
            },
            show_stacktrace: () => {
        ...
        },
        {
            ...
            display: (/** @type{string} */ x) => {
                if (pluto_actions.get_notebook().nbpkg?.enabled === false)
                    return default_rewriter.display(x);
                ...
                return (0, Preact_js_1.html) `<p>The package <strong>${package_name}.jl</strong> could not load because it failed to initialize.</p>
                    ...
                    ? null
                    : (0, Preact_js_1.html) ` <p>You might find useful information in the package installation log:</p>
                              <${PkgTerminalView_js_1.PkgTerminalView} value=${pkg_terminal_value} />`} `;
            },
            show_stacktrace: () => false,
        },
        default_rewriter,
    ...
    const [show_more, set_show_more] = (0, Preact_js_1.useState)(false);
    (0, Preact_js_1.useEffect)(() => {
        set_show_more(false);
    ...
    const limited_stacktrace = (limited ? stacktrace.slice(0, first_stack_from_here + 1) : stacktrace).filter((frame) => !(ignore_location(frame) && ignore_funccall(frame)));
    const first_package = get_first_package(limited_stacktrace);
    const [stacktrace_waiting_to_view, set_stacktrace_waiting_to_view] = (0, Preact_js_1.useState)(true);
    (0, Preact_js_1.useEffect)(() => {
        set_stacktrace_waiting_to_view(true);
    }, [msg, stacktrace, cell_id]);
    return (0, Preact_js_1.html) `<jlerror>
        ...
        : stacktrace_waiting_to_view
            ? (0, Preact_js_1.html) `<section class="stacktrace-waiting-to-view">
                  <button onClick=${() => set_stacktrace_waiting_to_view(false)}>Show stack trace...</button>
              </section>`
            : (0, Preact_js_1.html) `<section>
                  <div class="stacktrace-header">
                ...
                return (0, Preact_js_1.html) `<li class=${(0, ClassTable_js_1.cl)({ from_this_notebook, from_this_cell, important })}>
                              <div class="classical-frame">
                              ...
                              ${from_this_notebook ? (0, Preact_js_1.html) `<${LinePreview} frame=${frame} num_context_lines=${from_this_cell ? 1 : 2} />` : null}
                          </li>`;
            })}
                      ${limited
                ? (0, Preact_js_1.html) `<li class="important">
                                <a
                                    href="#"
                                    onClick=${(e) => {
                    set_show_more(true);
        ...
        ${pluto_actions.get_session_options?.()?.server?.dismiss_motivational_quotes !== true ? (0, Preact_js_1.html) `<${Motivation} stacktrace=${stacktrace} />` : null}
    </jlerror>`;
};
...
const Motivation = ({ stacktrace }) => {
    const msg = (0, Preact_js_1.useMemo)(() => {
    ...
    return msg == null ? null : (0, Preact_js_1.html) `<div class="dont-panic">${msg}</div>`;
};
