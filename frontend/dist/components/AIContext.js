...
const AIContext = ({ cell_id, current_code }) => {
    ...
    const [copied, setCopied] = (0, Preact_js_1.useState)(false);
    ...
    const [userQuestion, setUserQuestion] = (0, Preact_js_1.useState)(default_question);
    ...
    const formRef = (0, Preact_js_1.useRef)(null);
    (0, useEventListener_js_1.useEventListener)(formRef, "submit", (e) => {
        e.preventDefault();
        copyToClipboard();
...
`;
    ...
    const upstream_cellids = (0, SliderServerClient_js_1.upstream_recursive)(graph, [cell_id], { recursive });
...
const cell_output_to_plaintext = (/** @type {import("./Editor.js").CellResultData?} */ cell_result, /** @type {number} */ truncate_limit) => {
    ...
    if (cell_output.mime.includes("image"))
        return "<!-- Image -->";
        ...
        try {
            return JSON.stringify(cell_result, (key, value) => {
                if (typeof value === "string" && value.length > truncate_limit) {
                    return (value.substring(0, truncate_limit / 2) +
