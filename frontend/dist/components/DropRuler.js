 ...
 */
class DropRuler extends Preact_js_1.Component {
    ...
    componentDidMount() {
        ...
        document.addEventListener("dragstart", (e) => {
            ...
            if (target.matches("pluto-shoulder") && pe != null) {
                ...
                if (data)
                    e.dataTransfer.setData("text/pluto-cell", data);
                this.dropped = false;
                this.precompute_cell_edges();
                this.setState({
                    drag_start: true,
        ...
        const precompute_cell_edges_throttled = lodash_js_1.default.throttle(this.precompute_cell_edges, 4000, { leading: false, trailing: true });
        const update_drop_index_throttled = lodash_js_1.default.throttle(() => {
            this.setState({
    ...
    render() {
        const styles = this.state.drag_target
            ...
            : {};
        return (0, Preact_js_1.html) `<dropruler style=${styles}></dropruler>`;
    }
}
exports.DropRuler = DropRuler;
const argmin = (x) => {
