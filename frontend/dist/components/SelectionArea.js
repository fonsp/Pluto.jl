...
const SelectionArea = ({ on_selection, set_scroller, cell_order }) => {
    const mouse_position_ref = (0, Preact_js_1.useRef)();
    const is_selecting_ref = (0, Preact_js_1.useRef)(false);
    const element_ref = (0, Preact_js_1.useRef)(/** @type {HTMLElement?} */ (null));
    const [selection, set_selection] = (0, Preact_js_1.useState)(/** @type {{start: Coordinate2D, end: Coordinate2D}?} */ (null));
    (0, Preact_js_1.useEffect)(() => {
        ...
        let update_selection = in_request_animation_frame(({ pageX, pageY }) => {
            if (!is_selecting_ref.current || selection == null)
                return;
        ...
        const onkeydown = (e) => {
            if (e.key?.toLowerCase() === "a" && (0, KeyboardShortcuts_js_1.has_ctrl_or_cmd_pressed)(e)) {
    ...
    if (selection == null) {
        return (0, Preact_js_1.html) `<span ref=${element_ref}></span>`;
    }
