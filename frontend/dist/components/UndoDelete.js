"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.RecentlyDisabledInfo = exports.UndoDelete = void 0;
const Preact_js_1 = require("../imports/Preact.js");
const ClassTable_js_1 = require("../common/ClassTable.js");
const Scroller_js_1 = require("./Scroller.js");
const open_pluto_popup_js_1 = require("../common/open_pluto_popup.js");
const UndoDelete = ({ recently_deleted, on_click }) => {
    const [hidden, set_hidden] = (0, Preact_js_1.useState)(true);
    (0, Preact_js_1.useEffect)(() => {
        if (recently_deleted != null && recently_deleted.length > 0) {
            set_hidden(false);
            const interval = setTimeout(() => {
                set_hidden(true);
            }, 8000 * Math.pow(recently_deleted.length, 1 / 3));
            return () => {
                clearTimeout(interval);
            };
        }
    }, [recently_deleted]);
    let text = recently_deleted == null ? "" : recently_deleted.length === 1 ? "Cell deleted" : `${recently_deleted.length} cells deleted`;
    return (0, Preact_js_1.html) `
        <nav id="undo_delete" inert=${hidden} class=${(0, ClassTable_js_1.cl)({ hidden })}>
            ${text} (<a
                href="#"
                onClick=${(e) => {
        e.preventDefault();
        set_hidden(true);
        on_click();
    }}
                ><strong>UNDO</strong></a
            >)
        </nav>
    `;
};
exports.UndoDelete = UndoDelete;
/**
 * @param {{
 *  notebook: import("./Editor.js").NotebookData,
 *  recently_auto_disabled_cells: Record<string,[string,string]>,
 * }} props
 * */
const RecentlyDisabledInfo = ({ notebook, recently_auto_disabled_cells }) => {
    (0, Preact_js_1.useEffect)(() => {
        Object.entries(recently_auto_disabled_cells).forEach(([cell_id, reason]) => {
            (0, open_pluto_popup_js_1.open_pluto_popup)({
                type: "info",
                source_element: document.getElementById(reason[0]),
                body: (0, Preact_js_1.html) `<a
                        href=${`#${cell_id}`}
                        onClick=${(e) => {
                    (0, Scroller_js_1.scroll_cell_into_view)(cell_id);
                    e.preventDefault();
                    e.stopPropagation();
                }}
                        >Another cell</a
                    >${` has been disabled because it also defined `}<code class="auto_disabled_variable">${reason[1]}</code>.`,
            });
        });
    }, [recently_auto_disabled_cells]);
    return null;
};
exports.RecentlyDisabledInfo = RecentlyDisabledInfo;
