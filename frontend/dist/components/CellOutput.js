...
exports.ANSITextOutput = exports.generateCopyHeaderIdButton = exports.generateCopyCodeButton = exports.highlight = exports.RawHTMLContainer = exports.OutputBody = exports.PlutoImage = exports.CellOutput = void 0;
const Preact_js_1 = require("../imports/Preact.js");
const DOMPurify_js_1 = __importDefault(require("../imports/DOMPurify.js"));
...
const Bond_js_1 = require("../common/Bond.js");
const ClassTable_js_1 = require("../common/ClassTable.js");
const SetupCellEnvironment_js_1 = require("../common/SetupCellEnvironment.js");
...
const highlightjs_js_1 = __importDefault(require("../imports/highlightjs.js"));
...
const prettyAssignee = (assignee) => assignee && assignee.startsWith("const ") ? (0, Preact_js_1.html) `<span style="color: var(--cm-color-keyword)">const</span> ${assignee.slice(6)}` : assignee;
class CellOutput extends Preact_js_1.Component {
    constructor() {
        super();
        this.state = {
    ...
    render() {
        const rich_output = this.props.errored ||
            !this.props.body ||
...
let PlutoImage = ({ body, mime }) => {
    ...
    let imgref = (0, Preact_js_1.useRef)();
    ...
    return (0, Preact_js_1.html) `<img ref=${imgref} type=${mime} src=${""} />`;
};
exports.PlutoImage = PlutoImage;
/**
 * @param {{
 *  mime: string,
 * body: any,
...
const OutputBody = ({ mime, body, cell_id, persist_js_state = false, last_run_timestamp, sanitize_html = true }) => {
    ...
    switch (mime) {
        ...
        case "image/bmp":
        case "image/svg+xml":
            return (0, Preact_js_1.html) `<div><${exports.PlutoImage} mime=${mime} body=${body} /></div>`;
            break;
        case "text/html":
            ...
            if (body.startsWith("<!DOCTYPE") || body.startsWith("<html")) {
                return sanitize_html ? null : (0, Preact_js_1.html) `<${IframeContainer} body=${body} />`;
            }
            else {
                return (0, Preact_js_1.html) `<${exports.RawHTMLContainer}
                    cell_id=${cell_id}
                    body=${body}
        ...
        case "application/vnd.pluto.tree+object":
            return (0, Preact_js_1.html) `<div>
                <${TreeView_js_1.TreeView} cell_id=${cell_id} body=${body} persist_js_state=${persist_js_state} sanitize_html=${sanitize_html} />
            </div>`;
            break;
        case "application/vnd.pluto.table+object":
            return (0, Preact_js_1.html) `<${TreeView_js_1.TableView} cell_id=${cell_id} body=${body} persist_js_state=${persist_js_state} sanitize_html=${sanitize_html} />`;
            break;
        case "application/vnd.pluto.parseerror+object":
            return (0, Preact_js_1.html) `<div><${ErrorMessage_js_1.ParseError} cell_id=${cell_id} last_run_timestamp=${last_run_timestamp} ...${body} /></div>`;
            break;
        case "application/vnd.pluto.stacktrace+object":
            return (0, Preact_js_1.html) `<div><${ErrorMessage_js_1.ErrorMessage} cell_id=${cell_id} ...${body} /></div>`;
            break;
        case "application/vnd.pluto.divelement+object":
            return (0, TreeView_js_1.DivElement)({ cell_id, ...body, persist_js_state, sanitize_html });
            break;
        case "text/plain":
            if (body) {
                return (0, Preact_js_1.html) `<div><${exports.ANSITextOutput} body=${body} /></div>`;
            }
            else {
                return (0, Preact_js_1.html) `<div></div>`;
            }
            break;
        case null:
        case undefined:
        case "":
            ...
            break;
        default:
            return (0, Preact_js_1.html) `<pre title="Something went wrong displaying this object">🛑</pre>`;
            break;
    }
};
exports.OutputBody = OutputBody;
(0, PreactCustomElement_js_1.default)(exports.OutputBody, "pluto-display", ["mime", "body", "cell_id", "persist_js_state", "last_run_timestamp", "sanitize_html"]);
let IframeContainer = ({ body }) => {
    let iframeref = (0, Preact_js_1.useRef)();
    (0, Preact_js_1.useLayoutEffect)(() => {
    ...
    return (0, Preact_js_1.html) `<iframe
                    ...
                    let { node: new_node, result } = await execute_inside_script_tag_that_replaces(node, async (currentScript) => {
                        return await execute_dynamic_function({
                            environment: {
                                ...
                                _internal_getJSLinkResponse: (cell_id, link_id) => (input) => pluto_actions.request_js_link_response(cell_id, link_id, input).then(([success, result]) => {
                                    if (success)
                                        return result;
                                    throw result;
                                }),
                                getBoundElementValueLikePluto: Bond_js_1.get_input_value,
                                setBoundElementValueLikePluto: Bond_js_1.set_input_value,
                                getBoundElementEventNameLikePluto: Bond_js_1.eventof,
                                getNotebookMetadataExperimental: (key) => pluto_actions.get_notebook()?.metadata?.[key],
                                setNotebookMetadataExperimental: (key, value) => pluto_actions.update_notebook((notebook) => {
                                    notebook.metadata[key] = value;
                                }),
                                deleteNotebookMetadataExperimental: (key) => pluto_actions.update_notebook((notebook) => {
                                    delete notebook.metadata[key];
                                }),
                                ...(cell == null
                                    ? {}
                                    : {
                                        getCellMetadataExperimental: (key, { cell_id = null } = {}) => pluto_actions.get_notebook()?.cell_inputs?.[cell_id ?? cell.id]?.metadata?.[key],
                                        setCellMetadataExperimental: (key, value, { cell_id = null } = {}) => pluto_actions.update_notebook((notebook) => {
                                            notebook.cell_inputs[cell_id ?? cell.id].metadata[key] = value;
                                        }),
                                        deleteCellMetadataExperimental: (key, { cell_id = null } = {}) => pluto_actions.update_notebook((notebook) => {
                                            delete notebook.cell_inputs[cell_id ?? cell.id].metadata[key];
                                        }),
                                    }),
                                ...SetupCellEnvironment_js_1.observablehq_for_cells,
                                _: lodash_js_1.default,
                            },
                            code,
                        });
                    });
...
let RawHTMLContainer = ({ body, className = "", persist_js_state = false, last_run_timestamp, sanitize_html = true, sanitize_html_message = true }) => {
    ...
    let pluto_bonds = (0, Preact_js_1.useContext)(PlutoContext_js_1.PlutoBondsContext);
    let js_init_set = (0, Preact_js_1.useContext)(PlutoContext_js_1.PlutoJSInitializingContext);
    let previous_results_map = (0, Preact_js_1.useRef)(new Map());
    let invalidate_scripts = (0, Preact_js_1.useRef)(() => { });
    let container_ref = (0, Preact_js_1.useRef)(/** @type {HTMLElement?} */ (null));
    (0, Preact_js_1.useLayoutEffect)(() => {
        if (container_ref.current && pluto_bonds)
            (0, Bond_js_1.set_bound_elements_to_their_value)(container_ref.current.querySelectorAll("bond"), pluto_bonds);
    ...
    (0, Preact_js_1.useLayoutEffect)(() => {
        ...
        let html_content_to_set = sanitize_html
            ? DOMPurify_js_1.default.sanitize(body, {
                FORBID_TAGS: ["style"],
                ADD_ATTR: ["target"],
            })
            : body;
        ...
        if (sanitize_html_message && html_content_to_set !== body) {
            ...
            if (p2.documentElement.innerHTML !== p1.documentElement.innerHTML) {
                ...
                info_element.innerHTML = SafePreviewUI_js_1.SafePreviewSanitizeMessage;
        ...
        run(async () => {
            try {
                ...
                if (pluto_actions != null) {
                    ...
                    (0, Bond_js_1.set_bound_elements_to_their_value)(bond_nodes, pluto_bonds ?? {});
                    (0, Bond_js_1.add_bonds_listener)(bond_nodes, on_bond_value, pluto_bonds ?? {}, invalidation);
                    (0, Bond_js_1.add_bonds_disabled_message_handler)(bond_nodes, invalidation);
                }
    ...
    return (0, Preact_js_1.html) `<div class="raw-html-wrapper ${className}" ref=${container_ref}></div>`;
};
exports.RawHTMLContainer = RawHTMLContainer;
...
let highlight = (code_element, language) => {
    ...
    if (code_element.children.length === 0) {
        if (ENABLE_CM_HIGHLIGHTING &&
            ...
            const editorview = new CodemirrorPlutoSetup_js_1.EditorView({
                state: CodemirrorPlutoSetup_js_1.EditorState.create({
                    ...
                    extensions: [
                        (0, CodemirrorPlutoSetup_js_1.syntaxHighlighting)(CellInput_js_1.pluto_syntax_colors_julia),
                        (0, CodemirrorPlutoSetup_js_1.syntaxHighlighting)(CodemirrorPlutoSetup_js_1.defaultHighlightStyle, { fallback: true }),
                        CodemirrorPlutoSetup_js_1.EditorState.tabSize.of(4),
                        // TODO Other languages possibly?
                        ...(language === "julia" ? [CellInput_js_1.ENABLE_CM_MIXED_PARSER ? (0, mixedParsers_js_1.julia_mixed)() : (0, CodemirrorPlutoSetup_js_2.julia)()] : []),
                        CodemirrorPlutoSetup_js_1.EditorView.lineWrapping,
                        CodemirrorPlutoSetup_js_1.EditorView.editable.of(false),
                    ].filter((x) => x != null),
                }),
            });
            ...
            highlightjs_js_1.default.highlightElement(code_element);
        }
    }
};
...
const generateCopyHeaderIdButton = (/** @type {HTMLHeadingElement} */ header, /** @type {any} */ pluto_actions) => {
    ...
    const listener = (_e) => {
        ...
        if (launch_params?.isolated_cell_ids != null)
            return;
        const root = new URL(window.location.href);
        root.hash = "";
        const is_localhost_hostname = (hostname) => hostname === "localhost" || hostname === "127.0.0.1" || hostname === "0.0.0.0";
        if ((!launch_params || (launch_params.disable_ui && launch_params.notebook_id == null && launch_params.pluto_server_url == null)) &&
...
exports.generateCopyHeaderIdButton = generateCopyHeaderIdButton;
const ANSITextOutput = ({ body }) => {
    const has_ansi = /\x1b\[\d+m/.test(body);
    if (has_ansi) {
        return (0, Preact_js_1.html) `<${ANSIUpContents} body=${body} />`;
    }
    else {
        return (0, Preact_js_1.html) `<pre class="no-block"><code>${body}</code></pre>`;
    }
};
exports.ANSITextOutput = ANSITextOutput;
const ANSIUpContents = ({ body }) => {
    const node_ref = (0, Preact_js_1.useRef)(/** @type {HTMLElement?} */ (null));
    (0, Preact_js_1.useLayoutEffect)(() => {
        if (!node_ref.current)
            return;
        node_ref.current.innerHTML = (0, AnsiUp_js_1.ansi_to_html)(body);
    }, [body]);
    return (0, Preact_js_1.html) `<pre class="no-block"><code ref=${node_ref}></code></pre>`;
};
        ...
        container.querySelectorAll("code").forEach((code_element) => {
            code_element.classList.forEach((className) => {
                if (className.startsWith("language-") && !className.endsWith("undefined")) {
                    ...
                    (0, exports.highlight)(code_element, language);
                }
            });
        });
    }
    ...
    try {
        if (container.firstElementChild?.matches("div.markdown")) {
            container.querySelectorAll("pre > code").forEach((code_element) => {
                ...
                (0, exports.generateCopyCodeButton)(pre);
            });
            container.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((header_element) => {
                if (header_element.closest("table, pluto-display, bond"))
                    return;
                (0, exports.generateCopyHeaderIdButton)(/** @type {HTMLHeadingElement} */ (header_element), pluto_actions);
            });
        }
    }
    catch (err) {
