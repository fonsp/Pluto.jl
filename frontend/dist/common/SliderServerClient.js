...
exports.slider_server_actions = exports.nothing_actions = exports.is_noop_action = exports.upstream_recursive = exports.downstream_recursive = void 0;
const Binder_js_1 = require("./Binder.js");
const PlutoHash_js_1 = require("./PlutoHash.js");
const MsgPack_js_1 = require("./MsgPack.js");
...
const where_referenced = (/** @type {import("../components/Editor.js").CellDependencyGraph} */ graph, /** @type {Set<string> | string[]} */ vars) => {
    ...
    return all_cells.filter((cell_id) => lodash_js_1.default.some([...vars], (v) => Object.keys(graph[cell_id].upstream_cells_map).includes(v)));
};
const where_assigned = (/** @type {import("../components/Editor.js").CellDependencyGraph} */ graph, /** @type {Set<string> | string[]} */ vars) => {
    const all_cells = Object.keys(graph);
    return all_cells.filter((cell_id) => lodash_js_1.default.some([...vars], (v) => Object.keys(graph[cell_id].downstream_cells_map).includes(v)));
};
const downstream_recursive = (/** @type {import("../components/Editor.js").CellDependencyGraph} */ graph, starts, { recursive = true } = {}) => {
    ...
    while (ends.length > 0) {
        ...
        lodash_js_1.default.flatten(Object.values(graph[node].downstream_cells_map)).forEach((next_cellid) => {
...
exports.downstream_recursive = downstream_recursive;
const upstream_recursive = (/** @type {import("../components/Editor.js").CellDependencyGraph} */ graph, starts, { recursive = true } = {}) => {
    ...
    while (ends.length > 0) {
        const node = ends.splice(0, 1)[0];
        lodash_js_1.default.flatten(Object.values(graph[node].upstream_cells_map)).forEach((next_cellid) => {
            if (!deps.has(next_cellid)) {
                if (recursive)
                    ends.push(next_cellid);
                deps.add(next_cellid);
...
exports.upstream_recursive = upstream_recursive;
const disjoint = (a, b) => !lodash_js_1.default.some([...a], (x) => b.has(x));
const is_noop_action = (action) => action?.__is_noop_action === true;
exports.is_noop_action = is_noop_action;
...
const nothing_actions = ({ actions }) => Object.fromEntries(Object.entries(actions).map(([k, v]) => [
    k,
...
exports.nothing_actions = nothing_actions;
const slider_server_actions = ({ setStatePromise, launch_params, actions, get_original_state, get_current_state, apply_notebook_patches }) => {
    setStatePromise((0, immer_js_1.default)((state) => {
    ...
    const notebookfile_hash = fetch(new Request(launch_params.notebookfile, { integrity: launch_params.notebookfile_integrity }))
        ...
        .then(PlutoHash_js_1.plutohash_arraybuffer);
    notebookfile_hash.then((x) => console.log("Notebook file hash:", x));
    const bond_connections = notebookfile_hash
        .then((hash) => fetch((0, Binder_js_1.trailingslash)(launch_params.slider_server_url) + "bondconnections/" + hash))
        .then(assert_response_ok)
        .then((r) => r.arrayBuffer())
        .then((b) => (0, MsgPack_js_1.unpack)(new Uint8Array(b)));
    ...
    const request_bond_response = (0, PlutoHash_js_1.debounced_promises)(async () => {
        const base = (0, Binder_js_1.trailingslash)(launch_params.slider_server_url);
        ...
        const next_layers = [...(0, exports.downstream_recursive)(dep_graph, first_layer)];
        const cells_depending_on_explicits = lodash_js_1.default.uniq([...first_layer, ...next_layers]);
        ...
        const mybonds_filtered = Object.fromEntries(lodash_js_1.default.sortBy(Object.entries(mybonds).filter(([k, v]) => to_send.has(k)), ([k, v]) => k));
        const need_to_send_explicits = (() => {
            const _to_send_starts = where_assigned(dep_graph, to_send);
            const _depends_on_to_send = (0, exports.downstream_recursive)(dep_graph, _to_send_starts);
            return !disjoint(_to_send_starts, _depends_on_to_send);
        })();
        ///
        // PART: Update visual cell running status
        ///
        const update_cells_running = async (running) => await setStatePromise((0, immer_js_1.default)((state) => {
        ...
        if (explicit_bond_names.size > 0) {
            ...
            const packed = (0, MsgPack_js_1.pack)(mybonds_filtered);
            const packed_explicits = (0, MsgPack_js_1.pack)(Array.from(explicit_bond_names));
            let unpacked = null;
            try {
                ...
                let add_explicits = async (url) => {
                    ...
                    if (need_to_send_explicits)
                        if (!lodash_js_1.default.isEqual(explicit_bond_names, to_send))
                            u.searchParams.set("explicit", await (0, PlutoHash_js_1.base64url_arraybuffer)(packed_explicits));
                    return u;
                };
                ...
                const response = use_get
                    ? await fetch(await add_explicits(url + (await (0, PlutoHash_js_1.base64url_arraybuffer)(packed))), {
                        method: "GET",
                    }).then(assert_response_ok)
                ...
                unpacked = (0, MsgPack_js_1.unpack)(new Uint8Array(await response.arrayBuffer()));
    ...
    return {
        ...(0, exports.nothing_actions)({ actions }),
        set_bond: async (symbol, value) => {
            setStatePromise((0, immer_js_1.default)((state) => {
                state.notebook.bonds[symbol] = { value: value };
            }));
            if (mybonds[symbol] == null || !lodash_js_1.default.isEqual(mybonds[symbol].value, value)) {
                mybonds[symbol] = { value: lodash_js_1.default.cloneDeep(value) };
...
exports.slider_server_actions = slider_server_actions;
