...
export const package_status = ({ nbpkg, package_name, available_versions, is_disable_pkg }) => {
    ...
    const waiting_for_permission = nbpkg?.waiting_for_permission ?? false
    const busy = !waiting_for_permission && ((nbpkg?.busy_packages ?? []).includes(package_name) || !(nbpkg?.instantiated ?? true))
