...
const DocLink = ({ frame }) => {
  ...
  const installed =
    nb?.nbpkg?.installed_versions?.[frame.source_package ?? ""] != null;
...
const StackFrameFilename = ({ frame, cell_id }) => {
  ...
  if (frame_cell_id != null) {
    ...
    >
      ${(frame_cell_id == cell_id
        ? t("t_stack_frame_this_cell")
        : t("t_stack_frame_other_cell")
      ).replaceAll(" ", "\xa0")}${noline(line)
        ? null
    ...
    const file_line = html`<em
      >${frame.file.replace(/#@#==#.*/, "")}${noline(frame.line)
        ? null
        : `:${frame.line}`}</em
    >`;
    ...
    const text =
      sp != null
        ? html`<strong>${origin}</strong>${nbsp}→${nbsp}${file_line}`
        : file_line;

    const href = frame?.url?.startsWith?.("https") ? frame.url : null;
    return html`<a title=${frame.path} class="remote-url" href=${href}
      >${text}</a
    >`;
  }
};
...
const ignore_funccall = (/** @type {StackFrame} */ frame) => {
  ...
  if (
    frame.call === "macro expansion" &&
    !frame.file.includes("#@#==#") &&
    extract_cell_id(frame.file) != null
  )
    return true;
...
const Funccall = ({ frame }) => {
  ...
  const silly_to_hide =
    (frame.call_short.match(/…/g) ?? "").length <= 1 &&
    frame.call.length < frame.call_short.length + 7;

  const expanded =
    expanded_state ||
    (frame.call === frame.call_short &&
      frame.func === funcname_args(frame.call)[0]) ||
    silly_to_hide;
  ...
  const funcname_display = funcname.match(/^#\d+(#\d+)?$/)
    ? html`<abbr title=${t("t_anonymous_function_abbr")}
        >anonymous function</abbr
      >`
    : funcname;

  let inner = html`<strong>${funcname_display}</strong
    ><${HighlightCallArgumentNames} code=${call_funcname_args[1]} />`;
...
const LinePreview = ({ frame, num_context_lines = 2 }) => {
  ...
  if (cell_id) {
    let code = /** @type{import("./Editor.js").NotebookData?} */ (
      pluto_actions?.get_notebook()
    )?.cell_inputs[cell_id]?.code;
    ...
    if (code) {
      ...
      return html`<a
        ...
        ><div>
          <pre>
${lines.map((line, i) =>
              frame.line - 1 - num_context_lines <= i &&
              i <= frame.line - 1 + num_context_lines
                ? html`<${JuliaHighlightedLine}
                    code=${line}
                    i=${i}
                    frameLine=${i === frame.line - 1}
                  />`
                : null
            )}</pre
          >
...
const HighlightCallArgumentNames = ({ code }) => {
  ...
  useLayoutEffect(() => {
    if (code_ref.current) {
      const html = code.replaceAll(
        /([^():{},; ]*)::/g,
        "<span class='argument_name'>$1</span>::"
      );
...
const insert_commas_and_and = (/** @type {any[]} */ xs) =>
  xs.flatMap((x, i) =>
    i === xs.length - 1 ? [x] : i === xs.length - 2 ? [x, " and "] : [x, ", "]
  );
              ...
              html`<li
                ...
                onmouseenter=${() =>
                  cell_is_unedited(cell_id)
                    ? window.dispatchEvent(
                        new CustomEvent("cell_highlight_range", {
                          detail: { cell_id, from, to },
                        })
                      )
                    : null}
                onmouseleave=${() =>
                  window.dispatchEvent(
                    new CustomEvent("cell_highlight_range", {
                      detail: { cell_id, from: null, to: null },
                    })
                  )}
              >
                <div class="classical-frame">
                  ...
                  <div class="frame-source">
                    ${at}<${StackFrameFilename}
                      frame=${{ file: "#==#" + cell_id, line }}
                      cell_id=${cell_id}
                    />
                  </div>
                </div>
...
const cell_is_unedited = (cell_id) =>
  document.querySelector(`pluto-cell[id="${cell_id}"].code_differs`) == null;

const frame_is_important_heuristic = (
  frame,
  frame_index,
  limited_stacktrace,
  frame_cell_id
...
const AnsiUpLine = (/** @type {{value: string}} */ { value }) => {
  ...
  return value === ""
    ? html`<p><br /></p>`
    : html`<p ref=${node_ref}>
        ${did_ansi_up.current ? null : without_ansi_chars}
      </p>`;
};
...
export const ErrorMessage = ({
  ...
  const default_rewriter = {
    ...
    display: (/** @type{string} */ x) =>
      _.dropRightWhile(x.split("\n"), (s) => s === "").map(
        (line) => html`<${AnsiUpLine} value=${line} />`
      ),
  };
  const rewriters = [
    {
        ...
        if (x.includes("\n\nBoundaries: ")) {
          ...
          const split_hint = html`<p>
            <a
              ...
              >${t("t_split_this_cell_into_cells", {
    ...
    {
      pattern:
        /LoadError: cannot assign a value to variable workspace#\d+\..+ from module workspace#\d+/,
      display: () =>
        html`<p>
            Tried to reevaluate an <code>include</code> call, this is not
            supported. You might need to restart this notebook from the main
            menu.
          </p>
          <p>
            For a workaround, use the alternative version of
            <code>include</code> described here:
            <a
              target="_blank"
              href="https://github.com/fonsp/Pluto.jl/issues/115#issuecomment-661722426"
              >GH issue 115</a
            >
          </p>
          <p>
            In the future, <code>include</code> will be deprecated, and this
            will be the default.
          </p>`,
    },
    {
      ...
      display: (/** @type{string} */ x) =>
        x.split("\n").map((line) => {
          ...
          if (match) {
            ...
            let symbol_links = syms.map(
              (what) => html`<a href="#${encodeURI(what)}">${what}</a>`
            );

            const symbol_interp = localized_list_htl(symbol_links, syms, {
              type: "conjunction",
            });

            return html`<p>
              ${th("t_cyclic_references_among", { symbols: symbol_interp })}
            </p>`;
          } else {
      ...
      display: (/** @type{string} */ x) =>
        x.split("\n").map((line) => {
          ...
          if (match) {
            ...
            let symbol_links = syms.map((what) => {
              const onclick = (ev) => {
                const where = document.querySelector(
                  `pluto-cell:not([id='${cell_id}']) span[id='${encodeURI(
                    what
                  )}']`
                );
                ev.preventDefault();
            ...
            return html`<p>
              ${th("t_multiple_definitions_for", { symbols: symbol_interp })}
            </p>`;
          } else {
            return html`<p>${th("t_combine_cells_begin_block")}</p>`;
          }
    ...
    },
    {
      ...
      display: (/** @type{string} */ x) => {
        const notebook = /** @type{import("./Editor.js").NotebookData?} */ (
          pluto_actions?.get_notebook()
        );
        ...
        let module = (match?.[2] ?? "")
          .replaceAll("`", "")
          .replaceAll(/Main\.var\"workspace#\d+\"/g, "this notebook");
        const undefvar_is_from_upstream = Object.values(
          notebook?.cell_dependencies ?? {}
        ).some((map) => Object.keys(map.downstream_cells_map).includes(sym));

        if (
          Object.keys(erred_upstreams).length === 0 ||
          !undefvar_is_from_upstream
        ) {
          if (sym && module) {
            return html` <p>
                UndefVarError: <code>${sym}</code> not defined in ${module}.
              </p>
        ...
        const symbol_links = Object.keys(erred_upstreams).map((key) => {
          const onclick = (ev) => {
            ev.preventDefault();
            const where = document.querySelector(
              `pluto-cell[id='${erred_upstreams[key]}']`
            );
            where?.scrollIntoView();
          };
        ...
        const symbol_interp = localized_list_htl(
          symbol_links,
          Object.keys(erred_upstreams),
          { type: "disjunction" }
        );

        return html`<p>
          <em
            >${th("t_another_cell_defining_xs_contains_errors", {
              symbols: symbol_interp,
            })}</em
          >
        </p>`;
      },
      show_stacktrace: () => {
        const erred_upstreams = get_erred_upstreams(
          pluto_actions?.get_notebook(),
          cell_id
        );
    ...
    },
    {
      ...
      display: (/** @type{string} */ x) => {
        ...
        const pkg_terminal_value =
          pluto_actions?.get_notebook()?.nbpkg?.terminal_outputs?.[
            package_name
          ];

        return html`${th("t_package_could_not_load", { package: package_name })}
        ${th("t_package_could_not_load_things_you_could_try", {
  ...
  const first_stack_from_here = stacktrace.findIndex(
    (frame) => extract_cell_id(frame.file) != null
  );

  const limited =
    !show_more &&
    first_stack_from_here !== -1 &&
    first_stack_from_here < stacktrace.length - 1;

  const limited_stacktrace = (
    limited ? stacktrace.slice(0, first_stack_from_here + 1) : stacktrace
  ).filter((frame) => !(ignore_location(frame) && ignore_funccall(frame)));

  const first_package = get_first_package(limited_stacktrace);

  const [stacktrace_waiting_to_view, set_stacktrace_waiting_to_view] =
    useState(true);
  useEffect(() => {
    const from_another_cell =
      first_stack_from_here !== -1 &&
      extract_cell_id(stacktrace[first_stack_from_here].file) !== cell_id;
  ...
  return html`<jlerror>
    <div class="error-header">
      <secret-h1
        >${first_package == null
          ? t("t_error_message")
          : t("t_error_message_from_package", {
    ...
    ${stacktrace.length == 0 || !(matched_rewriter.show_stacktrace?.() ?? true)
      ...
      ? html`<section class="stacktrace-waiting-to-view">
          <button onClick=${() => set_stacktrace_waiting_to_view(false)}>
            ${t("t_show_stack_trace")}
          </button>
        </section>`
      : html`<section>
          <div class="stacktrace-header">
          ...
          <ol>
            ${limited_stacktrace.map((frame, frame_index) => {
              ...
              const important = frame_is_important_heuristic(
                frame,
                frame_index,
                limited_stacktrace,
                frame_cell_id
              ...
              return html`<li
                class=${cl({ from_this_notebook, from_this_cell, important })}
              >
                <div class="classical-frame">
                ...
                ${from_this_notebook
                  ? html`<${LinePreview}
                      frame=${frame}
                      num_context_lines=${from_this_cell ? 1 : 2}
                    />`
                  : null}
              </li>`;
            })}
    ...
    ${pluto_actions?.get_session_options?.()?.server
      ?.dismiss_motivational_quotes !== true
      ? html`<${Motivation} stacktrace=${stacktrace} />`
      : null}
  </jlerror>`;
};
...
const motivational_words = /** @type {string[]} */ (
  t("t_motivational_words_be_creative_and_write_as_many_as_you_want", {
    returnObjects: true,
  })
);
...
const Motivation = ({ stacktrace }) => {
  const msg = useMemo(() => {
    return Math.random() < motivational_word_probability
      ? motivational_words[
          Math.floor(Math.random() * motivational_words.length)
        ]
      : null;
  }, [stacktrace]);
...
const get_erred_upstreams = (
  ...
  if (notebook != null && notebook?.cell_results?.[cell_id]?.errored) {
    const referenced_variables = Object.keys(
      notebook.cell_dependencies[cell_id]?.upstream_cells_map
    );

    referenced_variables.forEach((key) => {
      if (!visited_edges.includes(key)) {
        visited_edges.push(key);
        const cells_that_define_this_variable =
          notebook.cell_dependencies[cell_id]?.upstream_cells_map[key];

        cells_that_define_this_variable.forEach((upstream_cell_id) => {
          let upstream_errored_cells =
            get_erred_upstreams(notebook, upstream_cell_id, visited_edges) ??
            {};
          ...
          if (
            Object.keys(upstream_errored_cells).length === 0 &&
            notebook.cell_results[upstream_cell_id].errored &&
            upstream_cell_id !== cell_id
          ) {
