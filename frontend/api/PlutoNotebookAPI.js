import {
  ...
  ws_address_from_base,
} from "https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@main/frontend/common/PlutoConnection.js";
import { empty_notebook_state } from "https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@main/frontend/editor.js";
import {
  applyPatches,
  produceWithPatches,
} from "https://cdn.jsdelivr.net/gh/fonsp/Pluto.jl@main/frontend/imports/immer.js";
...
export class Pluto {
   ...
   * @returns {Promise<Array<{notebook_id: string, path: string}>>} - Array of notebook info
   */
  async getRunningNotebooks() {
    try {
      ...
      const response = await temp_client.send(
        "get_all_notebooks",
        {},
        {},
        false
   ...
   * @returns {PlutoNotebook} - PlutoNotebook instance
   */
  notebook(notebook_id) {
    if (!this._notebooks.has(notebook_id)) {
      this._notebooks.set(
        notebook_id,
        new PlutoNotebook(this.ws_address, notebook_id)
      );
    }
   ...
   * @param {string} notebook_text - Pluto notebook text content
   * @returns {Promise<PlutoNotebook>} - Connected PlutoNotebook instance
   */
  async createNotebook(notebook_text) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to upload notebook: ${response.status} ${response.statusText}`
        );
      }
...
export class PlutoNotebook {
   ...
   * @param {boolean} [maybe_confirm=false] - Whether to show confirmation dialog for risky files
   ...
   * @returns {Object} - Current notebook state
   */
  getNotebookState() {
   ...
   * @returns {Object} - Cell input and result data
   */
  getCell(cell_id) {
   ...
   * @returns {Promise<Object>} - Response from backend if run=true
   */
  ...
  async setCellsAndRun(cell_ids) {
    ...
    const new_task = this._update_queue_promise.then(async () => {
      ...
      const [new_notebook, changes] = produceWithPatches(
        this.notebook_state,
        (notebook) => {
          for (let cell_id of cell_ids) {
            if (this.cell_inputs_local[cell_id]) {
              ...
              notebook.cell_inputs[cell_id].code =
                this.cell_inputs_local[cell_id].code;
            }
          }
        }
      ...
      try {
        ...
        if (response.message?.response?.update_went_well === "👎") {
          throw new Error(
            `Pluto update_notebook error: (from Julia: ${response.message.response.why_not})`
          );
        }
        ...
        // Run the cells
        const run_response = await this.client.send(
          "run_multiple_cells",
          { cells: cell_ids },
          { notebook_id: this.notebook_id }
        );
   ...
   * @returns {Promise<string>} - New cell ID
   */
  ...
  async deleteCells(cell_ids) {
    ...
    await this._update_notebook_state((notebook) => {
      ...
      notebook.cell_order = notebook.cell_order.filter(
        (cell_id) => !cell_ids.includes(cell_id)
      );
    });
  ...
  isIdle() {
    ...
    return !(
      this.pending_local_updates > 0 ||
      Object.values(this.notebook_state.cell_results).some(
        (cell) => cell.running || cell.queued
      )
    );
  }
  ...
  _handle_notebook_diff(message) {
    if (message?.counter != null) {
      if (message.counter <= this.last_update_counter) {
        console.error(
          "State update out of order",
          message.counter,
          this.last_update_counter
        );
        return;
      }
  ...
  async _update_notebook_state(mutate_fn) {
    ...
    const [notebook, changes] = produceWithPatches(
      this.notebook_state,
      (notebook) => {
        mutate_fn(notebook);
      }
