...
export const getAvailableLanguages = () => {
  ...
  const englishKeys = Object.keys(
    i18next.options.resources?.en?.translation || {}
  );
  const totalKeys = englishKeys.length;

  return languages.map((lang) => {
    const lang_entries = Object.entries(
      i18next.options.resources?.[lang]?.translation || {}
    ).filter(([key, value]) => value !== "");
    const completeness =
      totalKeys > 0 ? Math.round((lang_entries.length / totalKeys) * 100) : 100;

    return {
      code: lang,
      name: t(`t_language_name`, {
        lng: lang,
        fallbackLng: false,
        defaultValue: lang,
      }),
...
export const th = (key, insertions) => {
  ...
  const can_interpolate_directly = (value) =>
    typeof value === "string" ||
    typeof value === "number" ||
  ...
  const with_slots = t(key, {
    ...
    ...Object.fromEntries(
      Object.entries(insertions ?? {}).map(([key, value]) => [
        key,
        can_interpolate_directly(value) ? value : slot(key),
      ])
    ),
  });

  const slots = find_slots(with_slots);
  const slots_extended = [
    { start: 0, end: 0, name: "" },
    ...slots,
    { start: with_slots.length, end: with_slots.length, name: "" },
  ];

  // The strings inbetween slots, including an initial and final string (possibly empty).
  const string_parts = slots_extended
    .slice(1)
    .map((slot, i) => with_slots.slice(slots_extended[i].end, slot.start));
  ...
  return html(
    to_template_strings_array_cached(string_parts, cache_key),
    ...to_interpolate
  );
};
...
export const localized_list_htl = (elements, elements_strings, options) => {
  ...
  return html(
    to_template_strings_array_cached(strings, parts.length),
    ...interpolation_parts
  );
};
...
const to_template_strings_array_cached = (
  /** @type {string[]} */ strings,
  key
) => {
