...
class TestRunner {
  ...
  assertNotIncludes(content, substring, message) {
    if (content.includes(substring)) {
      throw new Error(
        message || `Expected content to NOT include: ${substring}`
      );
    }
  }
}
...
runner.test("should resolve basic includes", () => {
  ...
  runner.assertIncludes(
    ...
    "Should include content from test_shared.jl"
  );
  runner.assertIncludes(
    result,
    "# ===== Included from: ./test_shared.jl =====",
    "Should have include markers"
  );
  runner.assertIncludes(
    result,
    "# ===== End of ./test_shared.jl =====",
    "Should have end markers"
  );
});
...
runner.test("should ignore commented includes", () => {
  ...
  runner.assertIncludes(
    ...
    "Should preserve commented include"
  );
  runner.assertIncludes(
    result,
    '    # include("another_commented_file.jl")',
    "Should preserve commented include with spaces"
  );
  runner.assertIncludes(
    result,
    '#include("mixed_case.jl")',
    "Should preserve commented include without space"
  );

  // Should not try to resolve commented files
  runner.assertNotIncludes(
    result,
    "# Error resolving include: commented_file.jl",
    "Should not try to resolve commented files"
  );
});
...
runner.test("should handle nested includes", () => {
  ...
  runner.assertIncludes(
    result,
    "# ===== Included from: ./nested_include.jl =====",
    "Should resolve first level include"
  );
  runner.assertIncludes(
    result,
    "# ===== Included from: ./test_shared.jl =====",
    "Should resolve nested include"
  );
  runner.assertIncludes(
    result,
    "const TEST_CONSTANT = 42",
    "Should include content from deeply nested file"
  );
  ...
  runner.assertIncludes(
    ...
    "Should ignore commented includes in nested structure"
  );
});
...
runner.test("should handle missing files gracefully", () => {
  ...
  try {
    ...
    runner.assertIncludes(
      result,
      "# Error reading file:",
      "Should handle missing files with error message"
    );
    runner.assertIncludes(
      result,
      'print("test")',
...
runner.test("should preserve file structure and formatting", () => {
  ...
  runner.assertIncludes(
    result,
    "module TestModule",
    "Should preserve module declaration"
  );
