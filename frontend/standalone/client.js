...
} from "../common/PlutoConnection.js";
...
export class Host {
  ...
  worker(notebook_id) {
    if (!this._notebooks.has(notebook_id)) {
      this._notebooks.set(
        notebook_id,
        new Worker(this.ws_address, notebook_id)
      );
    }
  ...
  async createWorker(notebook_text) {
    try {
      ...
      if (!notebook) {
        throw new Error(
          `Notebook ${notebook_id} could not be created properly`
        );
      }
...
`);
      ...
      const link_id = this.notebook_state.cell_results[cell_id].output.body
        .split('", "')[1]
        .substr(0, 16);
      return this.client
        .send(
