            ...
            ${null!=window.plutoDesktop?(0,r.html)`<${s.FilePicker}
                  ...
                  >
                      ...
                      <a
                          ...
                          title=${e.path}
                          onClick=${o=>{t||(a(S(e.path,_)),u(e.path,{transitioning:!0}))}}
                          >${S(e.path,_)}</a
                      >
...
`,A=[{title:"Notebooks",tags:"everything"}],R=e=>e?.id??e.url,O=({sources:e,direct_html_links:t})=>{let[o,n]=(0,r.useState)({});(0,r.useEffect)(()=>{null!=e&&(l(!1),n({}),Promise.any(Array.from(new Set(e.map(R))).map(t=>M(e.filter(e=>R(e)===t).map(async e=>{let{url:o,integrity:n,valid_until:r}=e;if(null!=r&&new Date(r)<new Date)throw Error(`Source ${o} is expired with valid_until ${r}`);let a=await (await fetch(new Request(o,{integrity:n??void 0}))).json();if("1"!==a.format_version)throw Error(`Invalid format version: ${a.format_version}`);return[a,t,o]})).then(([e,t,o])=>{n(n=>({...n,[t]:{...e,source_url:o}}))}))).catch(e=>{console.error("All featured sources failed to load: ",e),(e?.errors??[]).forEach(e=>console.error(e)),l(!0)}))},[e]),(0,r.useEffect)(()=>{Object.entries(o).length>0&&console.log("Sources:",o)},[o]);let[a,l]=(0,r.useState)(!1);(0,r.useEffect)(()=>{setTimeout(()=>{l(!0)},8e3)},[]);let i=0===Object.entries(o).length,s=Array.from(new Set(e?.map(R)??[])).map(e=>o[e]).filter(e=>null!=e);return i&&a?L:(0,r.html)`
