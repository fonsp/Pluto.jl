...
if cat == "Ks*;lj"
	cm_md(raw"Oh no! Someone messed with my pretty introduction. Change the code above to give our cat a proper name!")
else
...
cm_md(raw"I feel like our cat needs a friend. Let's call them $(friend).")

# ╔═╡ a1a20314-9543-11ea-17de-0b658da18992
if !@isdefined friend
	cm_md(raw"Uh oh, what is this? I forgot to add a cell defining our friend. Can you do it for me?")
else
	cm_md(raw"**Well done!** $cat and $friend are both happy with your performance.")
end
...
cm_md(raw"Now, if you're like my grandma, you're feeding the entire neigborhood by yourself. Let's see how many cans of cat food you need to prepare.")

# ╔═╡ 270ac49e-9549-11ea-3ffd-71ddaee9f134
cm_md(raw"But what does `confusing_function` do? If you ever need help, click on 📚 **Live docs** in the lower right, and then place your cursor on the code you need help with. 

If you don't see it, then your screen is too small! Maybe you need to zoom out?")

# ╔═╡ e11e1660-9549-11ea-22f6-8bb53dc045fe
cm_md(raw"Now we know to prepare $(length(consumption)) cans. Let's stock up!")
...
if @isdefined cans_in_stock
	cm_md(raw"Actually, I have a hunch there will be another cat coming. Uncomment the code below (remove the #) to add one more can. Remember to run it after making the change!")
else
	cm_md(raw"**Whoopsie!** Because Pluto figures out execution order for you, it doesn't really make sense to assign to the same variable twice. A smarter way to plan ahead is to write `cans_in_stock = consumption` — Pluto will take care of updating everything.")
end
...
cm_md(raw"We have $(length(cans_in_stock)) cans of cat food, and our cats need $(length(consumption)). Try adding another cat to the neighborhood to see what changes!")

# ╔═╡ 6c8e2108-9550-11ea-014d-235770ed4771
cm_md(raw"## Saving cats and notebooks
...
if @isdefined scary_dog
	cm_md(raw" $scary_dog is terrorizing the neighborhood! We must do something about it!")
else
	cm_md(raw"You saved the neighborhood! Referencing `scary_dog` leads to an `UndefVarError`, as if it never even existed.")
end
...
cm_md(raw"Remember learning HTML in junior high? Now you can use it for something! Pluto lets you `@bind` variables to HTML elements. As always, every time you change something, Pluto knows what to update!")

# ╔═╡ f2c79746-9554-11ea-39ca-298fd09248ad
@bind power_level html"<input type='range'>"

# ╔═╡ 0b094cea-9556-11ea-268e-0d270fd04d56
cm_md(raw"The power level is $power_level, but we should do more than just say it - let's equip our cats with $power_level emoji!")
...
if !@isdefined power
	cm_md(raw"Uh oh! Pluto doesn't support multiple expressions per cell. This is a conscious choice - this restriction helps you write less buggy code once you get used to it. To fix the code, you can either split the above cell, or wrap it in a `begin ... end` block. Both work.")
else
	cm_md(raw"**Well done!** Your cats have powers now.")
end
...
cm_md(raw"## Final notes")

# ╔═╡ 4d0ebb46-9553-11ea-3431-2d203f594815
cm_md(raw"If anything about this introduction left you confused, something doesn't work, or you have a cool new idea - don't hesitate to contact us! You can do it right from this screen, using the `Instant feedback` form in the bottom right.")

# ╔═╡ d736e096-9553-11ea-3ba5-277afde1afe7
cm_md(raw"Also, if you were wondering where `confusing_function` came from, here you go! Remember that you, too, can place code wherever you like.")
...
cm_md(raw"**Have fun using Pluto!**
