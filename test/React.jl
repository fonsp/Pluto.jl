...
@testset "Reactivity" begin
    ...
    @testset "Basic $(parallel ? "distributed" : "single-process")" for parallel in [false, true]
        ...
        @test !haskey(WorkspaceManager.workspaces, notebook.notebookID)
        ...
        @testset "Stack traces" begin
            ...
            let
                ...
                if Pluto.can_insert_filename
                    ...
                    @test occursin(notebook.cells[12].cellID |> string, st["stacktrace"][4]["file"])
            ...
            let
                ...
                if Pluto.can_insert_filename
                    ...
                    @test occursin(notebook.cells[13].cellID |> string, st["stacktrace"][4]["file"])
                    @test occursin(notebook.path |> basename, st["stacktrace"][4]["file"])
                else
                    @test_broken false
                end
            ...
            let
                ...
                if Pluto.can_insert_filename
                    @test st["stacktrace"][4]["line"] == 3
                    @test occursin(notebook.cells[14].cellID |> string, st["stacktrace"][4]["file"])
                    @test occursin(notebook.path |> basename, st["stacktrace"][4]["file"])

                    @test st["stacktrace"][5]["line"] == 1
                    @test occursin(notebook.cells[15].cellID |> string, st["stacktrace"][5]["file"])
