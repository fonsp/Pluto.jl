...
ClientSession(id::Symbol, stream) =
    let
...
UpdateMessage(type::Symbol, message::Any, notebook::Notebook) =
    UpdateMessage(type, message, notebook, nothing, missing)

function clientupdate_cell_output(
    notebook::Notebook,
    cell::Cell;
...
function clientupdate_cell_input(
    notebook::Notebook,
    cell::Cell;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(
        :cell_input,
        Dict(:code => cell.code, :folded => cell.code_folded),
        notebook,
        cell,
        initiator,
    )
...
function clientupdate_cell_added(
    notebook::Notebook,
    cell::Cell,
    new_index::Integer;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(:cell_added, Dict(
        :index => new_index - 1, # 1-based index (julia) to 0-based index (js)
    ), notebook, cell, initiator)
end

function clientupdate_cell_deleted(
    notebook::Notebook,
    cell::Cell;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(:cell_deleted, Dict(), notebook, cell, initiator)
end

function clientupdate_cells_moved(
    notebook::Notebook,
    cells::Vector{Cell},
    new_index::Integer;
    initiator::Union{Initiator,Missing} = missing,
...
function clientupdate_cell_queued(
    notebook::Notebook,
    cell::Cell;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(:cell_queued, Dict(), notebook, cell, initiator)
end

function clientupdate_cell_running(
    notebook::Notebook,
    cell::Cell;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(:cell_running, Dict(), notebook, cell, initiator)
end

function clientupdate_cell_folded(
    notebook::Notebook,
    cell::Cell,
    folded::Bool;
    initiator::Union{Initiator,Missing} = missing,
)
    UpdateMessage(:cell_folded, Dict(:folded => folded), notebook, cell, initiator)
end
