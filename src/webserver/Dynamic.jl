...
Diffing is done using `immer.js` (frontend) and `src/webserver/Diffing.pluto.jl` (server). We wrote Diffing ourselves to match immer's functionality, and the cool thing is: **it is a Pluto notebook**! Since Pluto notebooks are `.jl` files, we can just `include` it in our module.
...
"""
module Diffing include("./Diffing.pluto.jl") end
...
Diffing.use_triple_equals_for_arrays[] = true
...
function send_notebook_changes!(🙋::ClientRequest; commentary::Any=nothing)
    ...
    for (_, client) in 🙋.session.connected_clients
        if client.connected_notebook !== nothing && client.connected_notebook.notebook_id == 🙋.notebook.notebook_id
            ...
            patches = Diffing.diff(current_dict, notebook_dict)
...
const effects_of_changed_state = Dict(
    "path" => function(; request::ClientRequest, patch::Diffing.ReplacePatch)
    ...
    "process_status" => function(; request::ClientRequest, patch::Diffing.ReplacePatch)
    ...
    "cell_inputs" => Dict(
        Wildcard() => function(cell_id, rest...; request::ClientRequest, patch::Diffing.JSONPatch)
            Diffing.applypatch!(request.notebook, patch)
    ...
    "cell_order" => function(; request::ClientRequest, patch::Diffing.ReplacePatch)
        Diffing.applypatch!(request.notebook, patch)
        [FileChanged]
    end,
    "bonds" => Dict(
        Wildcard() => function(name; request::ClientRequest, patch::Diffing.JSONPatch)
            name = Symbol(name)
            Diffing.applypatch!(request.notebook, patch)
            set_bond_values_reactive(
                ...
                is_first_value=patch isa Diffing.AddPatch,
                run_async=true,
            )
            # [BondChanged]
            return no_changes
...
responses[:update_notebook] = function response_update_notebook(🙋::ClientRequest)
    ...
    try
        notebook = 🙋.notebook
        patches = (Base.convert(Diffing.JSONPatch, update) for update in 🙋.body["updates"])
        ...
        for patch in patches
            Diffing.applypatch!(current_state_for_clients[🙋.initiator.client], patch)
        end
