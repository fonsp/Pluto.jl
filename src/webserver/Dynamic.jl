...
function change_remote_cellinput!(
    session::ServerSession,
    notebook::Notebook,
    cell::Cell,
    newcode;
    initiator::Union{Initiator,Missing} = missing,
)
    ...
    putnotebookupdates!(
        session,
        notebook,
        clientupdate_cell_input(notebook, cell, initiator = initiator),
    )
end
...
responses[:get_version] =
    (
        ...
        putclientupdates!(
            ...
            UpdateMessage(
                :versioninfo,
                Dict(:pluto => PLUTO_VERSION_STR, :julia => JULIA_VERSION_STR),
                nothing,
                nothing,
                initiator,
            ),
...
responses[:add_cell] =
    (
        ...
        putnotebookupdates!(
            session,
            notebook,
            clientupdate_cell_added(notebook, new_cell, new_index, initiator = initiator),
        )
        save_notebook(notebook)
    end

responses[:delete_cell] =
    (
        session::ServerSession,
        body,
        notebook::Notebook,
...
responses[:move_multiple_cells] =
    (
        ...
        putnotebookupdates!(
            session,
            notebook,
            clientupdate_cells_moved(notebook, cells, new_index, initiator = initiator),
        )
        save_notebook(notebook)
    end

responses[:change_cell] =
    (
        session::ServerSession,
        body,
        notebook::Notebook,
        ...
        putnotebookupdates!(
            session,
            notebook,
            clientupdate_cell_running(notebook, cell, initiator = initiator),
        )
...
responses[:fold_cell] =
    (
        ...
        putnotebookupdates!(
            session,
            notebook,
            clientupdate_cell_folded(notebook, cell, newfolded, initiator = initiator),
        )
    end
...
responses[:get_output] =
    (
        ...
        putclientupdates!(
            session,
            initiator,
            clientupdate_cell_output(notebook, cell, initiator = initiator),
        )
    end

responses[:get_all_cells] =
    (
        session::ServerSession,
        body,
        notebook::Notebook;
        ...
        update = UpdateMessage(
            :cell_list,
            Dict(:cells => [Dict(:cell_id => string(cell.cell_id)) for cell in notebook.cells]),
            nothing,
            nothing,
            initiator,
        )
...
responses[:set_bond] =
    (
        ...
        any_dependents =
            !isempty(where_assigned(notebook, notebook.topology, Set{Symbol}([bound_sym])))
        ...
        triggered_other_cells = if body["is_first_value"]
            ...
            eq_tester = :(
                try
                    !ismissing($bound_sym) && ($bound_sym == $new_val)
                catch
                    false
                end
            ) # not just a === comparison because JS might send back the same value but with a different type (Float64 becomes Int64 in JS when it's an integer.)
        ...
        putnotebookupdates!(
            session,
            notebook,
            UpdateMessage(:bond_update, body, notebook, nothing, initiator),
        )

        if triggered_other_cells
            function custom_deletion_hook(
                notebook::Notebook,
                to_delete_vars::Set{Symbol},
                funcs_to_delete::Set{Vector{Symbol}},
                to_reimport::Set{Expr};
                to_run::Array{Cell,1},
            )
