...
# Example
```julia
save_upload(some_notebook_data; filename_base="hello") == "~/.julia/jive_notebooks/hello 5.jl"
```
"""
...
function new(session::ServerSession; run_async=true, notebook_id::UUID=uuid1())
    ...
    notebook = if session.options.compiler.sysimage === nothing
        ...
        Notebook([Cell("import Pkg"), Cell("# This cell disables JIVEbook's package manager and activates the global environment. Click on ? inside the bubble next to Pkg.activate to learn more.\n# (added automatically because a sysimage is used)\nPkg.activate()"), Cell()])
    end
