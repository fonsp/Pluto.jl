...
function open(session::ServerSession, path::AbstractString; run_async=true, compiler_options=nothing, as_sample=false, save_to_path::Bool=true)
    if as_sample
    ...
    nb = load_notebook(tamepath(path), session.options.evaluation.run_notebook_on_load, save_to_path=save_to_path)
