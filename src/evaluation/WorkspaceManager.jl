...
import ..JIVEbook
import ..JIVEbook: Configuration, Notebook, Cell, ProcessStatus, ServerSession, ExpressionExplorer, pluto_filename, Token, withtoken, tamepath, project_relative_path, putnotebookupdates!, UpdateMessage
import ..JIVEbook.Status
import ..JIVEbook.PkgCompat
import ..Configuration: CompilerOptions, _merge_notebook_compiler_options, _convert_to_flags
import ..JIVEbook.ExpressionExplorer: FunctionName
import ..PlutoRunner
import Malt
...
process_preamble() = quote
    ...
    const pluto_boot_environment_path = $(JIVEbook.pluto_boot_environment_path[])
...
function start_relaying_self_updates((session, notebook)::SN, run_channel)
    while true
        try
            ...
            JIVEbook.run_reactive!(session, notebook, notebook.topology, notebook.topology, Cell[cell_to_run]; user_requested_run=false)
        catch e
...
function start_relaying_logs((session, notebook)::SN, log_channel)
    update_throttled, flush_throttled = JIVEbook.throttled(0.1) do
        JIVEbook.send_notebook_changes!(JIVEbook.ClientRequest(; session, notebook))
    end
    ...
    while true
        try
            ...
            JIVEbook.@asynclog update_throttled()
        catch e
...
function create_workspaceprocess(WorkerType; compiler_options=CompilerOptions(), status::Status.Business=Status.Business())::Malt.AbstractWorker
    ...
    if WorkerType === Malt.InProcessWorker
        ...
        if !(isdefined(Main, :PlutoRunner) && Main.PlutoRunner isa Module)
            # we make PlutoRunner available in Main, right now it's only defined inside this JIVEbook module.
