...
function run_reactive!(
    ...
    cells::Array{Cell,1};
    deletion_hook::Function = WorkspaceManager.delete_vars,
)::TopologicalOrder
    ...
    to_delete_vars =
        union!(Set{Symbol}(), (old_topology[cell].assignments for cell in old_runnable)...)
    to_delete_funcs =
        union!(Set{Vector{Symbol}}(), (keys(old_topology[cell].funcdefs) for cell in old_runnable)...)

    # get the new topological order
    new_order = topological_order(notebook, new_topology, union(cells, keys(old_order.errable)))
    to_run =
        setdiff(union(new_order.runnable, old_order.runnable), keys(new_order.errable))::Array{Cell,1} # TODO: think if old error cell order matters
    ...
    to_delete_vars =
        union!(to_delete_vars, (new_topology[cell].assignments for cell in new_runnable)...)
    to_delete_funcs =
        union!(to_delete_funcs, (keys(new_topology[cell].funcdefs) for cell in new_runnable)...)

    # delete new variables in case a cell errors (then the later cells show an UndefVarError)
    new_errable = keys(new_order.errable)
    to_delete_vars =
        union!(to_delete_vars, (new_topology[cell].assignments for cell in new_errable)...)
    to_delete_funcs =
        union!(to_delete_funcs, (keys(new_topology[cell].funcdefs) for cell in new_errable)...)
...
function update_save_run!(
    ...
    to_run_online = if !prerender_text
        ...
        to_run_offline =
            filter(c -> !c.running && is_just_text(new, c) && is_just_text(old, c), cells)
...
update_save_run!(session::ServerSession, notebook::Notebook, cell::Cell; kwargs...) =
    update_save_run!(session, notebook, [cell]; kwargs...)
