...
function run_reactive_core!(
    ...
    if !PlutoReactiveCore.is_resolved(new_topology)
    ...
    new_topology = PlutoReactiveCore.exclude_roots(new_topology, removed_cells)
    ...
    new_topology = PlutoReactiveCore.exclude_roots(new_topology, indirectly_deactivated)
    ...
    for (i, cell) in enumerate(to_run)
        ...
        if !PlutoReactiveCore.is_resolved(new_topology) && can_help_resolve_cells(new_topology, cell)
...
function update_save_run!(
	...
	if !session.options.evaluation.run_notebook_on_load && prerender_text
		...
		setup_cells = filter(notebook.cells) do c
			PlutoReactiveCore.cell_precedence_heuristic(notebook.topology, c) < DEFAULT_PRECEDENCE_HEURISTIC
		end
