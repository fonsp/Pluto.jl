...
function generate_plaintext(notebook, strmacrotrim::Union{String,Nothing}=nothing;
    ...
    for cell_id in notebook.cell_order
        ...
        push!(cell_strings, if raw
                  text = strip(scode[nextind(scode, 1, ltrim):prevind(scode, end, rtrim)], ['\n'])
                  ifelse(is_disabled(cell), textcomment, identity)(text)
              else
                  codewrapper(cell, is_disabled(cell))
              end)
    end
...
function generate_md(notebook)
    header = _ -> "<!-- Generated by Pluto $PLUTO_VERSION -->"
    codewrapper(cell, _) = string("```julia\n", cell.code, "\n```")
    textcomment(text) = string("<!-- ", text, " -->")
    generate_plaintext(notebook, "md"; header, codewrapper, textcomment)
end

function generate_org(notebook)
    function header(notebook)
        metainfo = ["# Generated by Pluto $PLUTO_VERSION"]
        fm = frontmatter(notebook)
        if !isempty(fm)
            fm = merge(_default_frontmatter, fm)
            for key in ("title", "description", "date", "tags")
                haskey(fm, key) && push!(metainfo, string("#+", key, ": ", fm[key]))
            end
        end
        join(metainfo, '\n')
    end
    codewrapper(cell, disabled) =
        string("#+begin_src julia",
               ifelse(disabled, " :eval no", ""),
               ifelse(cell.code_folded, " :exports results", ""),
               '\n', cell.code, "\n#+end_src")
    textcomment(text) = string("#+begin_comment\n", text, "\n#+end_comment")
    function footer(notebook)
        if notebook.nbpkg_ctx !== nothing
            ...
            string("* Julia Environment :noexport:\n** Packages\n\n#+begin_src toml :tangle Project.toml\n",
                   strip(ptoml_contents),
                   "\n#+end_src\n\n** Manifest\n\n#+begin_src toml :tangle Manifest.toml\n",
                   strip(mtoml_contents),
                   "\n#+end_src")
        end
    end
    generate_plaintext(notebook, "org"; header, codewrapper, textcomment, footer)
end
