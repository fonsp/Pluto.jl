...
function generate_html(notebook; kwargs...)::String
    ...
    frontmatter = frontmatter(notebook)
    header_html = isempty(frontmatter) ? "" : frontmatter_html(frontmatter) # avoid loading HypertextLiteral if there is no frontmatter
