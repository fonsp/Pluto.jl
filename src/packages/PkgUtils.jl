...
import Pkg
import ..JIVEbook
import ..JIVEbook: Notebook, save_notebook, load_notebook, load_notebook_nobackup, withtoken, Token, readwrite, PkgCompat
import ..JIVEbook.PkgCompat: project_file, manifest_file
...
ensure_has_nbpkg(notebook::Notebook) = if notebook.nbpkg_ctx === nothing
    ...
    error("""
    This notebook is not using JIVEbook's package manager. 
    This means that the notebook contains Pkg.activate or Pkg.add calls

    Open the notebook using JIVEbook to get started.
    """)
else
...
function write_dir_to_nb(dir::String, notebook::Notebook)
    ...
    notebook.nbpkg_ctx = JIVEbook.PkgCompat.create_empty_ctx()
...
function update_notebook_environment(path::String; kwargs...)
    JIVEbook.update_nbpkg(
        JIVEbook.ServerSession(),
        load_notebook_nobackup(path);
        kwargs...
    )
end
...
function testnb()
    ...
    readwrite(JIVEbook.project_relative_path("test","packages","nb.jl"), t)
    t
end
