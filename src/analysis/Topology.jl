...
function topological_order(
    ...
    function dfs(cell::Cell)
        ...
        referencers =
            where_referenced(notebook, topology, topology[cell].assignments) |> Iterators.reverse
...
function all_indirect_calls(
    topology::NotebookTopology,
    symstate::SymbolsState,
    found::Set{Vector{Symbol}} = Set{Vector{Symbol}}(),
)::Set{Vector{Symbol}}
