...
function make_workspace(notebook::Notebook, new_process = (ENV["PLUTO_WORKSPACE_USE_DISTRIBUTED"] == "true"))::Workspace
    ...
    workspaces[notebook.notebookID] = workspace
    return workspace
end
...
function get_workspace(notebook::Notebook)::Workspace
    if haskey(workspaces, notebook.notebookID)
        workspaces[notebook.notebookID]
    else
        workspaces[notebook.notebookID] = make_workspace(notebook)
    end
end
